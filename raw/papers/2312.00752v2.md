---
ingested: 2026-04-30
sha256: 288d2e89c0ec1b56361bee35b281546d3112a9a7fe3a46c61a9cd7a69c24431a
---
...
- **Synthetics.** On important synthetic tasks such as copying and induction heads that have been proposed as being key to large language models, Mamba not only solves them easily but can *extrapolate solutions indefinitely long* ( $>1\text{M}$  tokens).
...
![Diagram of the Selective State Space Model architecture. It shows an input sequence x_t being processed through a 'Project' block and a 'Selection Mechanism' to produce discrete parameters Delta_t. These parameters, along with the previous hidden state h_{t-1}, are inputs to the 'Discrete' block which contains matrices A, B_t, and C_t. The output of this block is the next hidden state h_t and the output y_t. A 'GPU SRAM' and 'GPU HBM' icon is shown on the right, indicating the hardware-aware state expansion.](1b7d539e02a202c2cf2d97698b911447_img.jpg)

Diagram of the Selective State Space Model architecture. It shows an input sequence x\_t being processed through a 'Project' block and a 'Selection Mechanism' to produce discrete parameters Delta\_t. These parameters, along with the previous hidden state h\_{t-1}, are inputs to the 'Discrete' block which contains matrices A, B\_t, and C\_t. The output of this block is the next hidden state h\_t and the output y\_t. A 'GPU SRAM' and 'GPU HBM' icon is shown on the right, indicating the hardware-aware state expansion.
...
**Discretization.** The first stage transforms the “continuous parameters”  $(\Delta, A, B)$  to “discrete parameters”  $(\bar{A}, \bar{B})$  through fixed formulas  $\bar{A} = f_A(\Delta, A)$  and  $\bar{B} = f_B(\Delta, A, B)$ , where the pair  $(f_A, f_B)$  is called a *discretization rule*. Various rules can be used such as the zero-order hold (ZOH) defined in equation (4).
...
**Computation.** After the parameters have been transformed from  $(\Delta, A, B, C) \mapsto (\bar{A}, \bar{B}, C)$ , the model can be computed in two ways, either as a **linear recurrence** (2) or a **global convolution** (3).
...
**Linear Time Invariance (LTI).** An important property of equations (1) to (3) is that the model’s dynamics are constant through time. In other words  $(\Delta, A, B, C)$ , and consequently  $(\bar{A}, \bar{B})$  as well, are fixed for all time-steps. This property is
...
We specifically choose  $s_B(x) = \text{Linear}_N(x)$ ,  $s_C(x) = \text{Linear}_N(x)$ ,  $s_\Delta(x) = \text{Broadcast}_D(\text{Linear}_L(x))$ , and  $\tau_\Delta = \text{softplus}$ , where  $\text{Linear}_d$  is a parameterized projection to dimension  $d$ . The choice of  $s_\Delta$  and  $\tau_\Delta$  is due to a connection to RNN gating mechanisms explained in Section 3.5.

![Figure 2: Three diagrams illustrating different sequence-to-sequence tasks. 1. Copying: Shows a sequence of input elements (colored squares) and output elements (white squares) with constant spacing. The solution is a direct copy of the input. 2. Selective Copying: Similar to Copying but with random spacing between input and output elements. 3. Induction Heads: Shows a sequence of input elements and output elements. The output elements are a subset of the input elements, with a question mark indicating the need to retrieve the correct element based on context.](547f726730e589392f239257a833ede3_img.jpg)

Figure 2: Three diagrams illustrating different sequence-to-sequence tasks. 1. Copying: Shows a sequence of input elements (colored squares) and output elements (white squares) with constant spacing. The solution is a direct copy of the input. 2. Selective Copying: Similar to Copying but with random spacing between input and output elements. 3. Induction Heads: Shows a sequence of input elements and output elements. The output elements are a subset of the input elements, with a question mark indicating the need to retrieve the correct element based on context.
...
1:  $A : (D, N) \leftarrow \text{Parameter}$   
     $\triangleright$  Represents structured  $N \times N$  matrix  
2:  $B : (B, L, N) \leftarrow \text{sg}_B(x)$   
3:  $C : (B, L, N) \leftarrow \text{sc}_B(x)$   
4:  $\Delta : (B, L, D) \leftarrow \tau_{\Delta}(\text{Parameter} + \text{s}_{\Delta}(x))$   
...
![Figure 3: Architecture diagrams showing the evolution from H3 to Gated MLP to Mamba. The H3 block has a Conv layer followed by an SSM layer with a skip connection, then a nonlinearity (X) and a linear projection. The Gated MLP block has a linear projection, a nonlinearity (X), and a gated path with a nonlinearity (sigma). The Mamba block combines these, with a linear projection, a nonlinearity (X), an SSM layer with a skip connection, a nonlinearity (sigma), and a final linear projection. A legend on the right defines the symbols: green rectangle for Linear projection, blue rectangle for Sequence transformation, and X for Nonlinearity (activation or multiplication).](3121ebddccf183ca63bb9781be440a7e_img.jpg)
...
We highlight the most important connection: the classical gating mechanism of RNNs is an instance of our selection mechanism for SSMs. We note that the connection between RNN gating and the discretization of continuous-time systems is well established (Funahashi and Nakamura 1993; Tallec and Olivier 2018). In fact, Theorem 1 is an improvement of Gu, Johnson, Goel, et al. (2021, Lemma 3.1) generalizing to the ZOH discretization and input-dependent gates (proof in Appendix C). More broadly,  $\Delta$  in SSMs can be seen to play a generalized role of the RNN gating mechanism. In line with prior work, we adopt the view that *discretization of SSMs is the principled foundation of heuristic gating mechanisms*.

**Theorem 1.** When  $N = 1$ ,  $A = -1$ ,  $B = 1$ ,  $s_\Delta = \text{Linear}(x_t)$ , and  $\tau_\Delta = \text{softplus}$ , then the selective SSM recurrence (Algorithm 2) takes the form

$$\begin{aligned} g_t &= \sigma(\text{Linear}(x_t)) \\ h_t &= (1 - g_t)h_{t-1} + g_t x_t. \end{aligned} \quad (5)$$
...
In our experiments, the  $\Delta$  parameter (which can be viewed as a bias term) is initialized to  $\tau_{\Delta}^{-1}(\text{Uniform}([0.001, 0.1]))$ , following prior work on SSMs (Gu, Johnson, Timsalina, et al. 2023).
...
![Figure 3: Induction Heads Extrapolation. A line graph showing Accuracy (y-axis, 0.0 to 1.0) versus Test Sequence Length (x-axis, log scale from 10^2 to 10^5). A vertical dashed line at approximately 10^2.5 marks the Train Length. Several models are plotted: MHA-Absolute (blue), MHA-RoPE (orange), MHA-xPos (green), H3 (red), Hyena (purple), Mamba (brown), Random (black), and Train Length (dashed). Most models show a sharp drop in accuracy after the train length, with MHA-xPos and MHA-Absolute maintaining higher accuracy than others at longer sequence lengths.](ff2492be4fa814905acbad18f261b8a5_img.jpg)

Figure 3: Induction Heads Extrapolation. A line graph showing Accuracy (y-axis, 0.0 to 1.0) versus Test Sequence Length (x-axis, log scale from 10^2 to 10^5). A vertical dashed line at approximately 10^2.5 marks the Train Length. Several models are plotted: MHA-Absolute (blue), MHA-RoPE (orange), MHA-xPos (green), H3 (red), Hyena (purple), Mamba (brown), Random (black), and Train Length (dashed). Most models show a sharp drop in accuracy after the train length, with MHA-xPos and MHA-Absolute maintaining higher accuracy than others at longer sequence lengths.
...
![Figure 4: Scaling Laws on The Pile. Two line graphs showing Perplexity (log scale) versus FLOPs (log scale) for sequence lengths 2048 (left) and 8192 (right). Models plotted include Hyena (blue), RWKV (orange), Transformer (red), RetNet (green), H3++ (purple), Transformer++ (brown), and Mamba (dark blue). In both graphs, Mamba shows a steeper downward slope, indicating better scaling, and achieves lower perplexity than other models as FLOPs increase. Transformer++ is shown as a baseline for comparison.](eb903413d070b64f45cd763804ba443f_img.jpg)

Figure 4: Scaling Laws on The Pile. Two line graphs showing Perplexity (log scale) versus FLOPs (log scale) for sequence lengths 2048 (left) and 8192 (right). Models plotted include Hyena (blue), RWKV (orange), Transformer (red), RetNet (green), H3++ (purple), Transformer++ (brown), and Mamba (dark blue). In both graphs, Mamba shows a steeper downward slope, indicating better scaling, and achieves lower perplexity than other models as FLOPs increase. Transformer++ is shown as a baseline for comparison.

Figure 4: (**Scaling Laws.**) Models of size  $\approx 125M \approx 1.3B$  parameters, trained on the Pile. Mamba scales better than all other attention-free models and is the first to match the performance of a very strong “Transformer++” recipe that has now become standard, particularly as the sequence length grows.
...
![Figure 5: Two line plots showing scaling laws on the HG38 dataset. The left plot, 'Scaling Laws on the Human Genome (HG38)', shows Perplexity vs. Parameters (log scale) for HyenaDNA, Mamba, and Transformer++. The right plot, 'Scaling Laws - Sequence Length (HG38)', shows Perplexity vs. Sequence Length for HyenaDNA 1.4M, Mamba 1.4M, and Mamba 7M.](dd0f5301a5a6dd7c319701302110de88_img.jpg)

The figure contains two line plots. The left plot, titled 'Scaling Laws on the Human Genome (HG38)', shows Perplexity on the y-axis (ranging from 2.7 to 3.1) against Parameters on a log scale on the x-axis (ranging from 10<sup>3</sup> to 10<sup>7</sup>). Three models are plotted: HyenaDNA (blue line), Mamba (orange line), and Transformer++ (red line). All three models show a decrease in perplexity as the number of parameters increases. Mamba's performance improves more rapidly than the others, eventually matching Transformer++ and HyenaDNA at approximately 40M parameters. The right plot, titled 'Scaling Laws - Sequence Length (HG38)', shows Perplexity on the y-axis (ranging from 2.75 to 3.00) against Sequence Length on a log scale on the x-axis (ranging from 10<sup>3</sup> to 10<sup>6</sup>). Three models are plotted: HyenaDNA 1.4M (blue line), Mamba 1.4M (orange line), and Mamba 7M (green line). HyenaDNA's perplexity increases as sequence length increases. In contrast, Mamba's perplexity decreases as sequence length increases, with the 7M parameter model showing the lowest perplexity across all sequence lengths.

Figure 5: Two line plots showing scaling laws on the HG38 dataset. The left plot, 'Scaling Laws on the Human Genome (HG38)', shows Perplexity vs. Parameters (log scale) for HyenaDNA, Mamba, and Transformer++. The right plot, 'Scaling Laws - Sequence Length (HG38)', shows Perplexity vs. Sequence Length for HyenaDNA 1.4M, Mamba 1.4M, and Mamba 7M.
...
![Figure 6: Finetuning Accuracy (Species DNA Classification) plot showing accuracy vs sequence length for HyenaDNA 1.4M, Mamba 1.4M, Mamba 7M, and Random models.](7c1f9e78e0f033d391b687f1652f6e47_img.jpg)

This line graph shows the finetuning accuracy for species DNA classification across different sequence lengths. The x-axis represents 'Sequence Length' on a logarithmic scale from  $10^3$  to  $10^6$ . The y-axis represents 'Accuracy' from 0.2 to 0.8. Four models are compared: HyenaDNA 1.4M (blue line), Mamba 1.4M (orange line), Mamba 7M (green line), and a Random baseline (dashed black line). The Random baseline stays at approximately 0.25 accuracy. HyenaDNA 1.4M starts at ~0.3, peaks at ~0.4 around  $10^4$ , dips to ~0.3 at  $10^5$ , and ends at ~0.55. Mamba 1.4M starts at ~0.35, dips to ~0.3 at  $10^4$ , and then rises to ~0.7. Mamba 7M starts at ~0.3, rises to ~0.4 at  $10^4$ , and then rises sharply to ~0.8 at  $10^6$ .

Figure 6: Finetuning Accuracy (Species DNA Classification) plot showing accuracy vs sequence length for HyenaDNA 1.4M, Mamba 1.4M, Mamba 7M, and Random models.
...
![Figure 7: Scaling Laws - Sequence Length (YouTubeMix) plot showing bits per byte vs sequence length for SaShiMi and Mamba models.](c2b98986bdf45e15707f6b2bd7ade2bd_img.jpg)

This line graph shows the scaling laws for sequence length on the YouTubeMix dataset. The x-axis represents 'Sequence Length' on a logarithmic scale from  $10^4$  to  $10^6$ . The y-axis represents 'Bits Per Byte' from 1.300 to 1.475. Two models are compared: SaShiMi (blue line) and Mamba (orange line). Both models show a decrease in bits per byte as sequence length increases. SaShiMi starts at ~1.46 and decreases to ~1.35. Mamba starts at ~1.44 and decreases more sharply to ~1.31.

Figure 7: Scaling Laws - Sequence Length (YouTubeMix) plot showing bits per byte vs sequence length for SaShiMi and Mamba models.
...
![Figure 8 (Left): Line graph titled 'Scan vs Convolution vs Attention time (A100 80GB PCIe)'. The y-axis is 'Time (ms)' on a log scale from 0.1 to 1000. The x-axis is 'Sequence length' from 512 to 512k. Four lines are shown: FlashAttention-2 (blue), Convolution (orange), Scan (PyTorch) (green), and Scan (ours) (red). FlashAttention-2 and Scan (ours) show a sharp increase in time as sequence length increases, while Convolution and Scan (ours) remain relatively flat. Scan (ours) is significantly faster than FlashAttention-2 and Scan (PyTorch) for sequence lengths above 2K.](fe753d01ad5fe6cf150018c958173c6d_img.jpg)

Figure 8 (Left): Line graph titled 'Scan vs Convolution vs Attention time (A100 80GB PCIe)'. The y-axis is 'Time (ms)' on a log scale from 0.1 to 1000. The x-axis is 'Sequence length' from 512 to 512k. Four lines are shown: FlashAttention-2 (blue), Convolution (orange), Scan (PyTorch) (green), and Scan (ours) (red). FlashAttention-2 and Scan (ours) show a sharp increase in time as sequence length increases, while Convolution and Scan (ours) remain relatively flat. Scan (ours) is significantly faster than FlashAttention-2 and Scan (PyTorch) for sequence lengths above 2K.
...
We perform a series of detailed ablations on components of our model, focusing on the setting of language modeling with size  $\approx 350\text{M}$  models at Chinchilla token counts (same setting as Figure 4).
...
| Mamba  | MHA+MLP | <b>1.850</b> | <b>1.37</b> | <b>5.63</b> | <b>58.23</b> | <b>0.62</b> |
| Mamba  | S4+MLP  | 1.853        | 1.07        | 6.05        | 73.34        | 0.55        |
| Mamba  | Mamba   | <b>1.852</b> | <b>0.94</b> | <b>6.26</b> | <b>88.54</b> | <b>0.52</b> |

![Figure 8 (Right): Bar chart titled 'Inference throughput on A100 80GB (prompt length 2048)'. The y-axis is 'Throughput (tokens / s)' from 0 to 1500. The x-axis is 'Batch size' from 1 to 128. Four bars are shown for each batch size: Mamba 1.4B (blue), Transformer 1.3B (orange), Mamba 6.9B (green), and Transformer 6.7B (red). Mamba 1.4B and Mamba 6.9B show significantly higher throughput than their Transformer counterparts across all batch sizes, with Mamba 1.4B achieving the highest throughput at larger batch sizes.](c260fa717b624233542a7116e022e771_img.jpg)

Figure 8 (Right): Bar chart titled 'Inference throughput on A100 80GB (prompt length 2048)'. The y-axis is 'Throughput (tokens / s)' from 0 to 1500. The x-axis is 'Batch size' from 1 to 128. Four bars are shown for each batch size: Mamba 1.4B (blue), Transformer 1.3B (orange), Mamba 6.9B (green), and Transformer 6.7B (red). Mamba 1.4B and Mamba 6.9B show significantly higher throughput than their Transformer counterparts across all batch sizes, with Mamba 1.4B achieving the highest throughput at larger batch sizes.
...
| -     | H3    | S6           | <b>8.95</b> | Mamba | Mamba | S6           | <b>8.69</b> |
...
- [29] Yassir Fathullah, Chunyang Wu, Yuan Shangguan, Junteng Jia, Wenhan Xiong, Jay Mahadeokar, Chunxi Liu, Yangyang Shi, Ozlem Kalinli, Mike Seltzer, and Mark J. F. Gales. "Multi-Head State Space Model for Speech Recognition". In: *Proc. INTERSPEECH 2023*. 2023, pp. 241–245. doi: [10.21437/Interspeech.2023-1036](https://doi.org/10.21437/Interspeech.2023-1036).
- [30] Karl J Friston, Lee Harrison, and Will Penny. "Dynamic Causal Modelling". In: *Neuroimage* 19.4 (2003), pp. 1273–1302.
- [31] Daniel Y Fu, Elliot I Epstein, Eric Nguyen, Armin W Thomas, Michael Zhang, Tri Dao, Atri Rudra, and Christopher Ré. "Simple Hardware-efficient Long Convolutions for Sequence Modeling". In: *The International Conference on Machine Learning (ICML)* (2023).
...
- [34] Leo Gao, Jonathan Tow, Stella Biderman, Sid Black, Anthony DiPofi, Charles Foster, Laurence Golding, Jeffrey Hsu, Kyle McDonell, Niklas Muenighoff, Jason Phang, Laria Reynolds, Eric Tang, Anish Thite, Ben Wang, Kevin Wang, and Andy Zou. *A Framework for Few-shot Language Model Evaluation*. Version v0.0.1. Sept. 2021. doi: [10.5281/zenodo.5371628](https://doi.org/10.5281/zenodo.5371628). url: <https://doi.org/10.5281/zenodo.5371628>.
...
- [69] Shahar Lutati, Itamar Zimmerman, and Lior Wolf. "Focus Your Attention (with Adaptive IIR Filters)". In: *arXiv preprint arXiv:2305.14952* (2023).
...
- [72] Soroush Mehri, Kundan Kumar, Ishaaan Gulrajani, Rithesh Kumar, Shubham Jain, Jose Sotelo, Aaron Courville, and Yoshua Bengio. "SampleRNN: An Unconditional End-to-End Neural Audio Generation Model". In: *The International Conference on Learning Representations (ICLR)*. 2017.
...
- [85] Zhen Qin, Xiaodong Han, Weixuan Sun, Bowen He, Dong Li, Dongxu Li, Yuchao Dai, Lingpeng Kong, and Yiran Zhong. "Toepplitz Neural Network for Sequence Modeling". In: *The International Conference on Learning Representations (ICLR)*. 2023.
...
- [100] Yutao Sun, Li Dong, Shaohan Huang, Shuming Ma, Yuqing Xia, Jilong Xue, Jianyang Wang, and Furu Wei. "Retentive network: A successor to transformer for large language models". In: *arXiv preprint arXiv:2307.08621* (2023).
...
- [106] Ashish Vaswani, Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N. Gomez, Łukasz Kaiser, and Illia Polosukhin. "Attention Is All You Need". In: *Advances in Neural Information Processing Systems (NeurIPS)*. 2017.
- [107] Eugene Vorontsov, Chihheb Trabelsi, Samuel Kadoury, and Chris Pal. "On Orthogonality and Learning Recurrent Networks with Long Term Dependencies". In: *International Conference on Machine Learning*. PMLR. 2017, pp. 3570–3578.
...
- They do not use state expansion ( $N = 1$ ) or selective  $B, C$  parameters, both of which are important for performance (Section 4.6).
...
*Proof of Theorem 1.* Consider a selective SSM (Algorithm 2) with  $N = 1$ ,  $A = -1$ ,  $B = 1$ ,  $s_\Delta = \text{Linear}(x_t)$ ,  $\tau_\Delta = \text{softplus}$ . The corresponding continuous-time SSM (1) is
...
$$\begin{aligned}\bar{A}_t &= \exp(\Delta A) = \frac{1}{1 + \exp(\text{Linear}(x_t))} = \sigma(-\text{Linear}(x_t)) \\ &= 1 - \sigma(\text{Linear}(x_t)) \\ \bar{B}_t &= (\Delta A)^{-1}(\exp(\Delta A) - I) \cdot \Delta B = -(\exp(\Delta A) - I) = 1 - \bar{A}_t \\ &= \sigma(\text{Linear}(x_t)).\end{aligned}$$
...
Table 11: (**Induction heads**.) Models are trained on sequence length  $2^8 = 256$ , and tested on various sequence lengths of  $2^6 = 64$  up to  $2^{20} = 1048576$ .  $\checkmark$  denotes perfect generalization accuracy, while  $\times$  denotes out of memory.

| MODEL    | PARAMS | TEST ACCURACY (%) AT SEQUENCE LENGTH |              |       |              |              |              |              |              |              |              |              |              |              |              |              |          |
|----------|--------|--------------------------------------|--------------|-------|--------------|--------------|--------------|--------------|--------------|--------------|--------------|--------------|--------------|--------------|--------------|--------------|----------|
|          |        | $2^6$                                | $2^7$        | $2^8$ | $2^9$        | $2^{10}$     | $2^{11}$     | $2^{12}$     | $2^{13}$     | $2^{14}$     | $2^{15}$     | $2^{16}$     | $2^{17}$     | $2^{18}$     | $2^{19}$     | $2^{20}$     |          |
| MHA-Abs  | 137K   | $\checkmark$                         | $\checkmark$ | 99.6  | 100.0        | 58.6         | 26.6         | 18.8         | 9.8          | 10.9         | 7.8          | $\times$     | $\times$     | $\times$     | $\times$     | $\times$     | $\times$ |
| MHA-RoPE | 137K   | $\checkmark$                         | $\checkmark$ | 100.0 | 83.6         | 31.3         | 18.4         | 8.6          | 9.0          | 5.5          | $\times$     | $\times$     | $\times$     | $\times$     | $\times$     | $\times$     | $\times$ |
| MHA-xPos | 137K   | $\checkmark$                         | $\checkmark$ | 100.0 | 99.6         | 67.6         | 25.4         | 7.0          | 9.0          | 7.8          | $\times$     | $\times$     | $\times$     | $\times$     | $\times$     | $\times$     | $\times$ |
| H3       | 153K   | $\checkmark$                         | $\checkmark$ | 100.0 | 80.9         | 39.5         | 23.8         | 14.8         | 8.2          | 5.9          | 6.6          | 8.2          | 4.7          | 8.2          | 6.3          | 7.4          |          |
| Hyena    | 69M*   | 97.7                                 | $\checkmark$ | 100.0 | $\checkmark$ | 44.1         | 12.5         | 6.6          | 5.1          | 7.0          | 5.9          | 6.6          | 6.6          | 5.9          | 6.3          | 9.8          |          |
| Mamba    | 74K    | $\checkmark$                         | $\checkmark$ | 100.0 | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ | $\checkmark$ |          |
...
**Mamba Architecture: Interleaving Blocks.** We test the effect of different architectural blocks combined with the Mamba block. We focus on the viewpoint that the Mamba block is simply the standard SwiGLU block with an extra  $\text{conv} \rightarrow \text{SSM}$  path added. This leads to two natural ablations:
...
![Figure 9: Scaling laws on The Pile (Sequence Length 2048). The figure contains two line plots. The left plot compares Mamba (blue), Mamba-MLP (purple), and Mamba-MHA (pink) architectures. The right plot compares Hyena (blue), Hyena+ (light blue), H3+ (green), and H3++ (dark green) architectures. Both plots show Perplexity (log scale) on the y-axis (ranging from 10^0 to 10^1) versus FLOPs (log scale) on the x-axis (ranging from 10^19 to 10^20). In both cases, the models show a linear decrease in perplexity as FLOPs increase on the log-log scale, indicating power-law scaling. The Hyena+ and H3++ models show lower perplexity than their base counterparts Hyena and H3+ respectively.](bafe3665fa89ba09857af5a2532c79fe_img.jpg)

Figure 9: Scaling laws on The Pile (Sequence Length 2048). The figure contains two line plots. The left plot compares Mamba (blue), Mamba-MLP (purple), and Mamba-MHA (pink) architectures. The right plot compares Hyena (blue), Hyena+ (light blue), H3+ (green), and H3++ (dark green) architectures. Both plots show Perplexity (log scale) on the y-axis (ranging from 10^0 to 10^1) versus FLOPs (log scale) on the x-axis (ranging from 10^19 to 10^20). In both cases, the models show a linear decrease in perplexity as FLOPs increase on the log-log scale, indicating power-law scaling. The Hyena+ and H3++ models show lower perplexity than their base counterparts Hyena and H3+ respectively.
...
We use a total batch size of  $2^{24} \approx 16M$  tokens per training step, for every sequence length (e.g. at length  $2^{20}$  there are 16 segments per batch and at length  $2^{10}$  there are 16384 segments per batch). This is a large batch size relative to the model size by usual LM standards, but note that a batch size of  $2^{25}$  is the minimum possible on a machine with 8 GPUs and sequence length of  $2^{20}$ , and that HyenaDNA used much larger batches of  $2^{28}$ .
...
Models are causal and therefore only the last element (across the sequence length) of the model’s output is used for the classification head. Note that we control for the total number of elements in the loss function per gradient step. The pretraining objective includes all positions across the sequence length, so that  $\text{batch\_size} \times \text{sequence\_length}$  is held constant; in other words, the batch size decreases as the sequence length increases. However, for a classification task, since only the last position enters the loss, the batch size itself is held constant. Note that this also means that fine-tuning models with longer sequence lengths is more computationally expensive.
...
![Figure 10: Two line graphs titled 'Audio Waveforms - SSM Parameterization'. The left graph shows 'Bits Per Byte' vs 'Sequence Length' for S4+MLP, Mamba (S6), complex, selective B/C, and selective Δ (Mamba-S4). The right graph shows the same for Mamba (S6), complex, selective B/C, and selective Δ (Mamba-S4). Both graphs show that as sequence length increases, bits per byte decrease. The left graph shows that S4+MLP has the highest bits per byte, while the right graph shows that Mamba (S6) has the highest bits per byte.](24b549c0c4b79c515b244625cf4f4978_img.jpg)

Figure 10: Two line graphs titled 'Audio Waveforms - SSM Parameterization'. The left graph shows 'Bits Per Byte' vs 'Sequence Length' for S4+MLP, Mamba (S6), complex, selective B/C, and selective Δ (Mamba-S4). The right graph shows the same for Mamba (S6), complex, selective B/C, and selective Δ (Mamba-S4). Both graphs show that as sequence length increases, bits per byte decrease. The left graph shows that S4+MLP has the highest bits per byte, while the right graph shows that Mamba (S6) has the highest bits per byte.
...
As a baseline, we implement a standard parallel scan in PyTorch with no kernel fusion. This requires materializing the parameters  $\bar{A}, \bar{B}, \bar{C}$  in HBM.
