...
source_url: 
ingested: 2026-04-30
sha256: e76d01dfa5f11feb662b911ef166ed90364d6ef080b232fca5a7c986a524b073
---
...
0.8
V
Lyapunov function value along simulated trajectories
...
arXiv:2109.14152v1  [cs.RO]  29 Sep 2021
...
stable neural-network controller together with a neural-
...
ics is
xt+1 = f(xt, ut) = φdyn(xt, ut) −φdyn(x∗, u∗) + x∗
(1a)
umin ≤ut ≤umax
(1b)
where xt ∈Rnx, ut ∈Rnu, umin and umax are the lower/upper
input limits. φdyn is a feedforward fully connected neural
network with leaky ReLU activation functions 2
3. x∗/u∗
...
function V (xt) : Rnx →R, such that the following Lyapunov
conditions are satisﬁed:
V (xt) > 0 ∀xt ∈S, xt ̸= x∗
(2a)
V (xt+1) −V (xt) ≤−ϵ2V (xt) ∀xt ∈S, xt ̸= x∗
(2b)
V (x∗) = 0
(2c)
where S is a compact sub-level set S = {xt|V (xt) ≤ρ}, and
ϵ2 > 0 is a given positive scalar. The Lyapunov conditions
in (2) guarantee that starting from any state inside S, the
state converges exponentially to the equilibrium state x∗,
and S is a region of attraction of the closed-loop system.
...
Lyapunov function V (·) as well as the control policy π(·),
...
For a general fully-connected neural network, the in-
put/output relationship in each layer is
zi =σ(Wizi−1 + bi), i = 1, . . . , n −1
(3a)
zn =Wnzn−1 + bn, z0 = x,
(3b)
where Wi, bi are the weights/biases of the i’th layer. The
activation function σ(·) is the leaky ReLU function shown
in Fig.3, as a piecewise linear function σ(y) = max(y, cy)
where 0 ≤c < 1 is a given scalar. If we suppose that for one
leaky ReLU neuron, the input y ∈R is bounded in the range
ylo ≤y ≤yup (where ylo < 0 and yup > 0), then we can use
...
β ∈{0, 1},
(4c)
where the binary variable β is active when y ≥0. Since the
only nonlinearity in the neural network (3) is the leaky ReLU
unit σ(·), by replacing it with constraints (4), the relationship
...
φV : Rnx →R as
V (xt) = φV (xt) −φV (x∗) + |R(xt −x∗)|1,
(5)
where R is a matrix with full column rank. |R(xt−x∗)|1 is the
1-norm of the vector R(xt −x∗). Eq. (5) guarantees V (x∗) =
0, hence condition (2c) is trivially satisﬁed. Notice that even
without the term |R(xt −x∗)|1 in (5), the Lyapunov function
would still satisfy V (x∗) = 0, but adding this 1-norm term
assists V (·) in satisfying the Lyapunov condition V (xt) >
0. As visualized in Fig 4, φV (xt) −φV (x∗) is a piecewise-
...
x∗, the term φV (xt)−φV (x∗) is a linear function of xt, which
will become negative away from x∗, violating the positivity
condition V (xt) > 0 ((2a)). To remedy this, we add the term
|R(xt −x∗)|1 to the Lyapunov function. Due to R being full-
...
Rnx →Rnu as
ut = π(xt) = clamp (φπ(xt) −φπ(x∗) + u∗, umin, umax) ,
(7)


Fig. 4: The term φV (xt) −φV (x∗) is a piecewise-afﬁne
...
the neighbourhood of x∗. By adding the 1-norm |R(xt−x∗)|1
(red lines), the Lyapunov function (blue lines) is at least locally
positive around x∗.
where clamp(·) clamps the value φπ(xt) −φπ(x∗) + u∗
...
strict inequalities constraints ≥and ≤, we change condition
(2a) to the following condition with ≥
V (xt) ≥ϵ1|R(xt −x∗)|1 ∀x ∈S,
(9)
where 0 < ϵ1 < 1 is a given positive scalar. Since R is full
...
the condition (9) and (2b) for a given bounded polytope B
around the equilibrium state. The veriﬁer solves the following
optimization problems
max
xt∈B ϵ1|R(xt −x∗)|1 −V (xt)
(10a)
max
xt∈B V (xt+1) −V (xt) + ϵ2V (xt),
(10b)
...
variable xt since both V (xt) and |R(xt −x∗)|1 are piecewise-
...
afﬁne function within a bounded domain (B in this case)
...
1-norm in |R(xt −x∗)|1 and the clamp function in the control
policy.
...
for any state in B, is that the Lyapunov condition holds for
many sampled states within B. Hence we could reduce a
...
surrogate loss function on a training set X containing sampled
states. The training set X grows after each MIP solve by
appending the counter examples generated from the MIP solve.
Since the MIP (10a) and the MIP (10b) generate different
counter examples, we keep two separate training sets X1 and
X2 for MIP (10a) and MIP (10b) respectively.
We design a surrogate loss function for X1, X2 to measure
...
2), 0),
(11b)
We denote η1(X1) and η2(X2) as the vectors whose i’th
entry is the violation on the i’th sample η1(xi
1) and η2(xi
2)
respectively, then our surrogate function is deﬁned as
lossθ(X1, X2) = |η1(X1)|p + |η2(X2)|p,
(12)
where | · |p denotes the p-norm of a vector, such as 1-norm
(mean of the violation), ∞−norm (maximal of the violation)
and 4−norm (a smooth approximation of the ∞−norm). The
...
violation on just the sampled states, the batched gradient
...
candidate Lyapunov function V , and training sets X1, X2.
...
problem can then be simpliﬁed to
γ(θ) = max
s
cT
θ s + dθ
...
in the MIP, including xt and other slack variables. The
...
bounded polytope B, we compute an inner approximation of
the region of attraction for the closed-loop system. (Notice
that the veriﬁed region B is not a region of attraction, since
it’s not an invariant set, while the sub-level sets of V are
guaranteed to be invariant). One valid inner approximation is
the largest sub-level set S = {xt|V (xt) ≤ρ} contained inside
the veriﬁed region B, as illustrated in Fig. 6. Since we already
obtained the Lyapunov function V (xt) in the previous section,
we only need to ﬁnd the largest value of ρ such that S ⊂B.
Equivalently we can ﬁnd ρ through the following optimization
problem
ρ = min
xt∈∂B V (xt),
(15)
where the compact set ∂B is the boundary of the polytopic
region B, and the constraint xt ∈∂B can be formulated as
Fig. 6: An inner approx-
imation of the region of
attraction S is the largest
sub-level set V (xt) ≤ρ
contained inside the veri-
ﬁed region B, where the
...
face of the polytope B). As explained previously, the Lyapunov
...
through regression. To stabilize the pendulum at the top
...
largest level set within the veriﬁed box region B (black dashed
...
We start with a small box region 0.8π ≤θ ≤1.2π, −1 ≤
˙θ ≤1, and then gradually increase the veriﬁed region. We
...
smaller box 0.8π ≤θ ≤1.2π, −1 ≤˙θ ≤1, both algorihm
1 and 2 converge within a few minutes. For the larger box
0 ≤θ ≤2π, −5 ≤˙θ ≤5, both algorithms converge within 3
hours.
B. 2D quadrotor
We synthesize a stabilizing controller and a Lyapunov
function for the 2D quadrotor model used in [51]. Again
we ﬁrst train a neural network φdyn to approximate the
Lagrangian dynamics. Our goal is to steer the quadrotor
to hover at the origin. In Fig.9 we visualize the snap-
shots of the quadrotor stabilized by our neural-network con-
troller. We veriﬁed the Lyapunov conditions in the region
[−0.75, −0.75, −0.5π, −4, −4, −2.75] ≤[x, z, θ, ˙x, ˙z, ˙θ] ≤
...
a small box region [−0.1, −0.1, −0.1π, −0.5, −0.5, −0.3] ≤
[x, z, θ, ˙x, ˙z, ˙θ] ≤[0.1, 0.1, 0.1π, 0.5, 0.5, 0.3], both algorithms
...
gion [−8, −8] ≤[ ˙x, ˙z] ≤[8, 8], with [x, z, θ, ˙θ] ﬁxed to
[−0.75, 0.3, 0.3π, 2] (left), and [0.75, 0.5, −0.4π, 2] (right).
...
0.3
V
Simulated with Lagrangian dynamics
0.0
2.5
5.0
7.5
...
0.0004
0.0005
MIP (10a) optimal cost
Loss curve with Algorithm 1
0
250
500
750
...
solely reﬂects the opinions and conclusions of its authors and
...
Andrea Peruffo.
Formal synthesis of lyapunov neural
...
Manfred
Morari,
George J Pappas, and Victor M Preciado.
Learning
...
[14] Hongkai Dai, Benoit Landry, Marco Pavone, and Russ
Tedrake.
Counter-example guided synthesis of neural
...
2020.
[18] Matteo Fischetti and Jason Jo.
...
Some controls
applications of sum of squares programming. In 42nd
...
230–239, 2019.
[38] Daniel Mellinger and Vijay Kumar.
Minimum snap
...
ing control lyapunov functions from counterexamples
...
dynamics, the control policy and the Lyapunov function
respectively. All the networks use leaky ReLU activation
...
For the l1 norm constraint on x ∈Rn
y = |x|1
(16)
where x is bounded elementwisely as l ≤x ≤u, we
...
zi ≥xi, zi ≥−xi
(17b)
zi ≤xi + 2li(αi −1), zi ≤2uiαi −xi
(17c)
α ∈{0, 1}
(17d)


where we assume l < 0, u > 0 (the case when l ≥0 or u ≤0
is trivial).
...
V (x∗) = 0, V (x) > 0 ∀x ̸= x∗
(20)
if and only if for a given positive scalar ϵ > 0 and a given full
column-rank matrix R, there exists another piecewise-afﬁne
function ¯V (x) : Rn →R satisfying
¯V (x∗) = 0, ¯V (x) ≥ϵ|R(x −x∗)|1 ∀x ̸= x∗
(21)
...
we get ¯V (·). Intuitively this scalar could be found as the
smallest ratio between V (x) and ϵ|R(x −x∗)|1. Formally,
given a unit length vector d ∈Rn, we deﬁne a scalar function
φd(t) = V (x∗+ td), this scalar function φ(·) : R →R is
also piecewise-afﬁne, as it is just the value of V (·) along
the direction d. Likewise we deﬁne another scalar function
ψd(t) = ϵ|R(td)|1 which is just the value of the right-hand
side of (21) along the direction d. Since both φd(t), ψd(t)
are piecewise-afﬁne and positive deﬁnite, the minimal ratio
ζ(d) = mint̸=0 φd(t)/ψd(t) is strictly positive. Moreover,
consider the value κ = mindT d=1 ζ(d), since the domain
{d|dT d = 1} is a compact set, and the minimal of a positive
function ζ(d) on a compact set is still positive, hence κ > 0.
As a result, setting ¯V (·) = V (·)/κ will satisfy (21), because
¯V (x) =
¯V (x∗+ td) = V (x∗+ td)/κ = φd(t)/κ ≥
φd(t)/ζ(d) ≥φd(t)/(φd(t)/ψd(t)) = ψd(t) = ϵ|R(x −x∗)|1
where we choose d = (x −x∗)/|x −x∗| and t = |x −x∗|.
...
Fig. 14. If the dynamics were linear, then this quadratic
...
100
V = xTSx
LQR Lyapunov function value along simulated trajectories
