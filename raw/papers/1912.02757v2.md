---
ingested: 2026-04-30
sha256: a6595640de90adfc7e5831e159479d6753731200983ea8f7509818eb348c073d
---
...
Consider a typical classification problem, where  $\mathbf{x}_n \in \mathbb{R}^D$  denotes the  $D$ -dimensional features and  $y_n \in [1, \dots, K]$  denotes the class label. Assume we have a parametric model  $p(y|\mathbf{x}, \boldsymbol{\theta})$  for the conditional distribution where  $\boldsymbol{\theta}$  denotes weights and biases of a neural network, and  $p(\boldsymbol{\theta})$  is a prior distribution over parameters. The Bayesian posterior over parameters is given by  $p(\boldsymbol{\theta}|\{\mathbf{x}_n, y_n\}_{n=1}^N) \propto p(\boldsymbol{\theta}) \prod_{n=1}^N p(y_n|\mathbf{x}_n, \boldsymbol{\theta})$ .

Computing the exact posterior distribution over  $\boldsymbol{\theta}$  is computationally expensive (if not impossible) when  $p(y_n|\mathbf{x}_n, \boldsymbol{\theta})$  is a deep neural network (NN). A variety of approximations have been developed for *Bayesian neural networks*, including Laplace approximation [MacKay, 1992], Markov chain Monte Carlo methods [Neal, 1996, Welling and Teh, 2011, Springenberg et al., 2016], variational Bayesian methods [Graves, 2011, Blundell et al., 2015, Louizos and Welling, 2017, Wen et al., 2018] and Monte-Carlo dropout [Gal and Ghahramani, 2016, Srivastava et al., 2014]. While computing the posterior is challenging, it is usually easy to perform maximum-a-posteriori (MAP) estimation, which corresponds to a mode of the posterior. The MAP solution can be written as the minimizer of the following loss:

$$\hat{\boldsymbol{\theta}}_{\text{MAP}} = \arg \min_{\boldsymbol{\theta}} L(\boldsymbol{\theta}, \{\mathbf{x}_n, y_n\}_{n=1}^N) = \arg \min_{\boldsymbol{\theta}} -\log p(\boldsymbol{\theta}) - \sum_{n=1}^N \log p(y_n|\mathbf{x}_n, \boldsymbol{\theta}). \quad (1)$$
...
<sup>3</sup>We use the term ‘mode’ to refer to unique functions  $f_\theta(\mathbf{x})$ . Due to weight space symmetries, different parameters can correspond to the same function, i.e.  $f_{\theta_1}(\mathbf{x}) = f_{\theta_2}(\mathbf{x})$  even though  $\theta_1 \neq \theta_2$ .
...
First, we compute the similarity between different checkpoints along a single trajectory. In Figure 2(a), we plot the cosine similarity in weight space, defined as  $\cos(\theta_1, \theta_2) = \frac{\theta_1^\top \theta_2}{\|\theta_1\| \|\theta_2\|}$ . In Figure 2(b), we plot the disagreement in function space, defined as the fraction of points the checkpoints disagree on, that is,  $\frac{1}{N} \sum_{n=1}^N [f(x_n; \theta_1) \neq f(x_n; \theta_2)]$ , where  $f(x; \theta)$  denotes the class label predicted by the network for input  $x$ . We observe that the checkpoints along a trajectory are largely similar both in the weight space and the function space. Next, we evaluate how diverse the final solutions from different random initializations are. The functions from different initialization are different, as demonstrated

<sup>4</sup>[https://keras.io/examples/cifar10\\_resnet/](https://keras.io/examples/cifar10_resnet/)

![Figure 2: Results using SmallCNN on CIFAR-10. (a) Cosine similarity of weights heatmap showing high similarity along the diagonal. (b) Disagreement of predictions heatmap showing low disagreement along the diagonal. (c) t-SNE plot of predictions for three trajectories, showing distinct clusters for each trajectory.](5d92d5c9cc01a262b0389d138caa9aea_img.jpg)
...
by the similarity plots in Figure 3. Comparing this with Figures 2(a) and 2(b), we see that functions within a single trajectory exhibit higher similarity and functions across different trajectories exhibit much lower similarity.

Next, we take the predictions from different checkpoints along the individual training trajectories from multiple initializations and compute a t-SNE plot [Maaten and Hinton, 2008] to visualize their similarity in function space. More precisely, for each checkpoint we take the softmax output for a set of examples, flatten the vector and use it to represent the model’s predictions. The t-SNE algorithm is then used to reduce it to a 2D point in the t-SNE plot. Figure 2(c) shows that the functions explored by different trajectories (denoted by circles with different colors) are far away, while functions explored within a single trajectory (circles with the same color) tend to be much more similar.

![Figure 3: Results on CIFAR-10 using two different architectures. (a) Results using SmallCNN: Cosine similarity and disagreement heatmaps. (b) Results using ResNet20v1: Cosine similarity and disagreement heatmaps. Both show high similarity and low disagreement within a trajectory.](b7251436a2a3c0d1c00c3e935df2a8f5_img.jpg)

Figure 3: Results on CIFAR-10 using two different architectures. (a) Results using SmallCNN: Cosine similarity and disagreement heatmaps. (b) Results using ResNet20v1: Cosine similarity and disagreement heatmaps. Both show high similarity and low disagreement within a trajectory.

Figure 3: Results on CIFAR-10 using two different architectures. For each of these architectures, the left subplot shows the cosine similarity between different solutions in weight space, and the right subplot shows the fraction of labels on which the predictions from different solutions disagree. In general, the weight vectors from two different initializations are essentially orthogonal, while their predictions are approximately as dissimilar as any other pair.

### 4.2 Similarity of Functions Within Subspace from Each Trajectory and Across Trajectories

In addition to the checkpoints along a trajectory, we also construct subspaces based on each individual trajectory. Scalable variational Bayesian methods typically approximate the distribution of weights along the training trajectory, hence visualizing the diversity of functions between the subspaces helps understand the difference between Bayesian neural networks and ensembles. We use a representative set of four subspace sampling methods: Monte Carlo dropout, a diagonal Gaussian approximation, a low-rank covariance matrix Gaussian approximation and a random subspace approximation. Unlike dropout and Gaussian approximations which assume a parametric form for the variational posterior, the random subspace method explores all high-quality solutions within the subspace and hence could be thought of as a non-parametric variational approximation to the posterior. Due to space constraints, we do not consider Markov Chain Monte Carlo (MCMC) methods in this work: Zhang et al. [2020] show that popular stochastic gradient MCMC (SGMCMC) methods may not explore multiple modes and propose cyclic SGMCMC. We compare diversity of random initialization and cyclic SGMCMC in Appendix C. In the descriptions of the methods, let  $\theta_0$  be the optimized weight-space solution (the weights and biases of our trained neural net) around which we will construct the subspace.

- **Random subspace sampling:** We start at an optimized solution  $\theta_0$  and choose a random direction  $v$  in the weight space. We step in that direction by choosing different values of  $t$  and looking at predictions at configurations  $\theta_0 + tv$ . We repeat this for many random directions  $v$ .
- **Dropout subspace:** We start at an optimized solution  $\theta_0$  apply dropout with a randomly chosen  $p_{\text{keep}}$ , evaluate predictions at  $\text{dropout}_{p_{\text{keep}}}(\theta_0)$  and repeat this many times with different  $p_{\text{keep}}$ .
- **Diagonal Gaussian subspace:** We start at an optimized solution  $\theta_0$  and look at the most recent iterations of training proceeding it. For each trainable parameter  $\theta_i$ , we calculate the mean  $\mu_i$  and standard deviation  $\sigma_i$  independently for each parameter, which corresponds to a diagonal covariance matrix. This is similar to SWAG-diagonal [Maddox et al., 2019]. To sample solutions from the subspace, we repeatedly draw samples where each parameter independently as  $\theta_i \sim \mathcal{N}(\mu_i, \sigma_i)$ .
- **Low-rank Gaussian subspace:** We follow the same procedure as the diagonal Gaussian subspace above to compute the mean  $\mu_i$  for each trainable parameter. For the covariance, we use a rank- $k$  approximation, by calculating the top  $k$  principal components of the recent weight vectors  $\{v_i \in \mathbb{R}^{\text{params}}\}_k$ . We sample from a  $k$ -dimensional normal distribution and obtain the weight configurations as  $\theta \sim \mu + \sum_i \mathcal{N}(0^k, 1^k)v_i$ . Throughout the text, we use the terms low-rank and PCA Gaussian interchangeably.

![Figure 4: Four t-SNE plots showing validation set predictions for SmallCNN on CIFAR-10. The plots are titled 'Runs & random subspaces', 'Runs & dropout subspaces', 'Runs & diagonal normal subspaces', and 'Runs & low-rank normal subspaces'. Each plot shows clusters of points representing different runs, with colored squares indicating the subspace generation method. The clusters are distinct, showing that different subspaces lead to different prediction-space neighborhoods.](e0d425c8e4eef259e4c52d81426d93fa_img.jpg)

Figure 4: Four t-SNE plots showing validation set predictions for SmallCNN on CIFAR-10. The plots are titled 'Runs & random subspaces', 'Runs & dropout subspaces', 'Runs & diagonal normal subspaces', and 'Runs & low-rank normal subspaces'. Each plot shows clusters of points representing different runs, with colored squares indicating the subspace generation method. The clusters are distinct, showing that different subspaces lead to different prediction-space neighborhoods.
...
![Figure 5: Three plots showing the radial loss landscape for MediumCNN on CIFAR-10. The left plot shows 'Accuracy' along weight space directions 1 and 2, with a color scale from 0.0 to 0.8. The middle plot shows 'Function similarity to optimum 1' with a color scale from 0.00 to 0.90. The right plot shows 'Function similarity to optimum 2' with a color scale from 0.00 to 0.90. All plots show a radial landscape with a sharp peak at the origin (0,0) and a plateau at the optima. The legend indicates: Init 1 (red circle), Path 1 (red line), Opt 1 (red star), Init 2 (blue circle), Path 2 (blue line), Opt 2 (blue star), Origin (cyan circle), and Gauss x100.0 (magenta line).](b15e3860e0c96ed16ce77f032da6f107_img.jpg)

Figure 5: Three plots showing the radial loss landscape for MediumCNN on CIFAR-10. The left plot shows 'Accuracy' along weight space directions 1 and 2, with a color scale from 0.0 to 0.8. The middle plot shows 'Function similarity to optimum 1' with a color scale from 0.00 to 0.90. The right plot shows 'Function similarity to optimum 2' with a color scale from 0.00 to 0.90. All plots show a radial landscape with a sharp peak at the origin (0,0) and a plateau at the optima. The legend indicates: Init 1 (red circle), Path 1 (red line), Opt 1 (red star), Init 2 (blue circle), Path 2 (blue line), Opt 2 (blue star), Origin (cyan circle), and Gauss x100.0 (magenta line).
...
The diversity score quantifies the difference of two functions (a base solution and a sampled one), by measuring fraction of datapoints on which their predictions differ. We chose this approach due to its simplicity; we also computed the KL-divergence and other distances between the output probability distributions, leading to equivalent conclusions. Let  $d_{\text{diff}}$  denote the fraction of predictions on which the two functions differ. It is 0 when the two functions make identical class predictions, and 1 when they differ on every single example. To account for the fact that the lower the accuracy of a function, the higher its potential  $d_{\text{diff}}$  due to the possibility of the wrong answers being random and uncorrelated between the two functions, we normalize this by  $(1 - a)$ , where  $a$  is the accuracy of the sampled solution. We also derive idealized lower and upper limits of these curves (showed in dashed lines) by perturbing the reference solution's predictions (lower limit) and completely random predictions at a given accuracy (upper limit), see Appendix D for a discussion.

![Figure 6: Diversity versus accuracy plots for 3 models trained on CIFAR-10: SmallCNN, MediumCNN, and ResNet20v1. Each plot shows the fraction of disagreement (1-D accuracy) on the y-axis versus test accuracy on the x-axis. The plots compare various subspace sampling methods (random subspace, dropout subspace, simple gaussian, rank-4 gaussian) against independently initialized and optimized solutions (red stars) and the baseline optimum (green star). The independently initialized optima achieve better diversity vs accuracy trade-off than the subspace sampling methods.](ac99eff233b8fe51d30f499e7413c345_img.jpg)

Figure 6: Diversity versus accuracy plots for 3 models trained on CIFAR-10: SmallCNN, MediumCNN, and ResNet20v1. Each plot shows the fraction of disagreement (1-D accuracy) on the y-axis versus test accuracy on the x-axis. The plots compare various subspace sampling methods (random subspace, dropout subspace, simple gaussian, rank-4 gaussian) against independently initialized and optimized solutions (red stars) and the baseline optimum (green star). The independently initialized optima achieve better diversity vs accuracy trade-off than the subspace sampling methods.
...
![Figure 7: Diversity vs. accuracy plots for ResNet20v1 on CIFAR-100 (a) and ResNet50v2 on ImageNet (b). Both plots show the fraction of disagreement (1-D accuracy) on the y-axis versus test accuracy on the x-axis. The plots compare various subspace sampling methods (random subspace, dropout subspace, simple gaussian, rank-4 gaussian) against independently initialized and optimized solutions (red stars) and the baseline optimum (green star). The independently initialized optima achieve better diversity vs accuracy trade-off than the subspace sampling methods.](ed2fa033a401564314cdc32fe9732935_img.jpg)
...
![Figure 8: Two line plots showing the performance of various methods on CIFAR-10 as a function of ensemble size. The left plot shows 'Test accuracy' (ranging from 0.70 to 0.78) and the right plot shows 'Test Brier' (ranging from 0.30 to 0.46). Both plots compare 'Single' (red line with circles) and 'Ensemble' (shaded area) methods across ensemble sizes 2, 4, 6, 8, 10, 12, and 14. Other methods shown as horizontal dashed lines are: Original (brown), Weight Avg (orange), Diag Gauss (yellow), KLR Gauss (green), Random (purple), and PCA Random (pink). In the left plot, accuracy increases with ensemble size, with the Ensemble method reaching the highest accuracy. In the right plot, the Brier score decreases as ensemble size increases, with the Ensemble method achieving the lowest score.](4279c8be6ec4ed56f4b3349be98bb426_img.jpg)

Figure 8: Two line plots showing the performance of various methods on CIFAR-10 as a function of ensemble size. The left plot shows 'Test accuracy' (ranging from 0.70 to 0.78) and the right plot shows 'Test Brier' (ranging from 0.30 to 0.46). Both plots compare 'Single' (red line with circles) and 'Ensemble' (shaded area) methods across ensemble sizes 2, 4, 6, 8, 10, 12, and 14. Other methods shown as horizontal dashed lines are: Original (brown), Weight Avg (orange), Diag Gauss (yellow), KLR Gauss (green), Random (purple), and PCA Random (pink). In the left plot, accuracy increases with ensemble size, with the Ensemble method reaching the highest accuracy. In the right plot, the Brier score decreases as ensemble size increases, with the Ensemble method achieving the lowest score.
...
The diversity versus accuracy plot compares diversity to a reference solution, but it is also important to also look at the diversity between *multiple samples of the same method*, as this will effectively determine the efficiency of the method in terms of the bias-variance trade-off. Function space diversity is particularly important to avoid overconfident predictions under dataset shift, as averaging over similar functions would not reduce overconfidence. To visualize this, we draw 5 samples of each method and compute the average Jensen-Shannon divergence between their predictions, defined as  $\sum_{m=1}^M KL(p_{\theta_m}(y|x) || \bar{p}(y|x))$  where  $KL$  denotes the Kullback-Leibler divergence and  $\bar{p}(y|x) = (1/M) \sum_m p_{\theta_m}(y|x)$ . Right subplot of Figure 9 shows the results on CIFAR-10-C for increasing corruption intensity. We observe that Jensen-Shannon divergence is the highest between independent random initializations, and lower for subspace sampling methods; the difference is

![Figure 9: Results using MediumCNN on CIFAR-10-C for varying levels of corruption intensity. The figure contains three line plots. The left plot shows Accuracy (y-axis, 0.75 to 0.95) vs. Corruption Intensity (x-axis, 0 to 4). The medium plot shows Brier score (y-axis, 0 to 0.7) vs. Corruption Intensity. The right plot shows Jensen-Shannon divergence (y-axis, 0.0 to 0.35) vs. Corruption Intensity. All plots compare Original, Weight Avg, Drop Outs, MC Dropout, Subspace, and Ensembles methods. Ensembles consistently show the highest accuracy and lowest Brier score and divergence across all corruption levels.](c3c305cefbac2e7b13be34ab87054d1e_img.jpg)

Figure 9: Results using MediumCNN on CIFAR-10-C for varying levels of corruption intensity. The figure contains three line plots. The left plot shows Accuracy (y-axis, 0.75 to 0.95) vs. Corruption Intensity (x-axis, 0 to 4). The medium plot shows Brier score (y-axis, 0 to 0.7) vs. Corruption Intensity. The right plot shows Jensen-Shannon divergence (y-axis, 0.0 to 0.35) vs. Corruption Intensity. All plots compare Original, Weight Avg, Drop Outs, MC Dropout, Subspace, and Ensembles methods. Ensembles consistently show the highest accuracy and lowest Brier score and divergence across all corruption levels.
...
higher under dataset shift, which explains the findings of Ovadia et al. [2019] that deep ensembles outperform other methods under dataset shift. We also observe similar trends when testing on an OOD dataset such as SVHN: JS divergence is 0.384 for independent runs, 0.153 for within-trajectory, 0.155 for random sampling, 0.087 for rank-5 PCA Gaussian and 0.034 for diagonal Gaussian.

**Results on ImageNet** To illustrate the effect on another challenging dataset, we repeat these experiments on ImageNet [Deng et al., 2009] using *ResNet50v2* architecture. Due to computational constraints, we do not evaluate PCA subspace on ImageNet. Figure 10 shows results on ImageNet test set (zero corruption intensity) and ImageNet-C for increasing corruption intensities. Similar to CIFAR-10, random subspace performs best within subspace sampling methods, and provides complementary benefits to random initialization. We empirically observed that the relative gains of WA (or subspace sampling) are smaller when the individual models converge to a better optima within each mode. Carefully choosing which points to average, e.g. using cyclic learning rate as done in fast geometric ensembling [Garipov et al., 2018] can yield further benefits.

![Figure 10: Results using ResNet50v2 on ImageNet test and ImageNet-C for varying corruptions. The figure consists of two rows of bar charts. The top row shows Accuracy (y-axis, 0.0 to 0.6) and the bottom row shows Brier score (y-axis, 0.0 to 0.8). Each row has six subplots for corruption intensities 0, 1, 2, 3, 4, and 5. Each subplot compares Single, Ensemble 2, and Ensemble 4 methods for Original, Weight Avg, Drop Outs, and Random corruptions. Ensembles generally maintain higher accuracy and lower Brier scores than single models as corruption intensity increases.](ecb25d766719ce041cf4cc390791a098_img.jpg)

Figure 10: Results using ResNet50v2 on ImageNet test and ImageNet-C for varying corruptions. The figure consists of two rows of bar charts. The top row shows Accuracy (y-axis, 0.0 to 0.6) and the bottom row shows Brier score (y-axis, 0.0 to 0.8). Each row has six subplots for corruption intensities 0, 1, 2, 3, 4, and 5. Each subplot compares Single, Ensemble 2, and Ensemble 4 methods for Original, Weight Avg, Drop Outs, and Random corruptions. Ensembles generally maintain higher accuracy and lower Brier scores than single models as corruption intensity increases.
...
- Jost Tobias Springenberg, Aaron Klein, Stefan Falkner, and Frank Hutter. Bayesian optimization with robust Bayesian neural networks. In *NeurIPS*, 2016.
...
- Yeming Wen, Paul Vicol, Jimmy Ba, Dustin Tran, and Roger Grosse. Flipout: Efficient pseudo-independent weight perturbations on mini-batches. In *ICLR*, 2018.
...
- Stanislav Fort and Adam Scherlis. The Goldilocks zone: Towards better understanding of neural network loss landscapes. In *AAAI*, 2019.
...
- Yuvai Netzer, Tao Wang, Adam Coates, Alessandro Bissacco, Bo Wu, and Andrew Y Ng. Reading Digits in Natural Images with Unsupervised Feature Learning. In *NeurIPS Workshop on Deep Learning and Unsupervised Feature Learning*, 2011.
...
![Figure S1: Radial loss landscape cut between the origin and two independent optima. (a) Accuracy along the radial loss-landscape cut for training and validation sets. (b) Function-space similarity to optima 1 and 2.](ff2492be4fa814905acbad18f261b8a5_img.jpg)

Figure S1 consists of two subplots. Subplot (a) shows 'Accuracy along the radial loss-landscape cut' for training and validation sets. The x-axis is 'Towards optimum 1' and 'Towards optimum 2', ranging from -1.5 to 1.5. The y-axis is 'Towards optimum 2', ranging from -1.5 to 1.5. The color scale represents accuracy, ranging from 0.44 to 0.54. Subplot (b) shows 'Function-space similarity' to optima 1 and 2. The x-axis is 'Towards optimum 1' and 'Towards optimum 2', ranging from -1.5 to 1.5. The y-axis is 'Towards optimum 2', ranging from -1.5 to 1.5. The color scale represents similarity, ranging from 0.00 to 0.04.

Figure S1: Radial loss landscape cut between the origin and two independent optima. (a) Accuracy along the radial loss-landscape cut for training and validation sets. (b) Function-space similarity to optima 1 and 2.

Figure S1: *Results using MediumCNN on CIFAR-10*: Radial loss landscape cut between the origin and two independent optima and the predictions of models on the same plane.

Next, we construct a low-loss tunnel between different optima using the procedure proposed by Fort and Jastrzebski [2019], which is a simplification of the procedures proposed in Garipov et al. [2018] and Draxler et al. [2018]. As shown in Figure S2(a), we start at the linear interpolation point (denoted by the black line) and reach the closest point on the manifold by minimizing the training loss. The minima of the training loss are denoted by the yellow line in the manifolds. Figure S2(b) confirms that the tunnel is indeed low-loss. This also confirms the findings of [Garipov et al., 2018, Fort and Jastrzebski, 2019] that while solutions along the tunnel have similar loss, they are dissimilar in function space.

![Figure S2: (a) Cartoon illustration of a low-loss tunnel between two optima. (b) Loss, accuracy, and label difference along the tunnel.](b712e7522f1bb7135730c7d1abb46d43_img.jpg)

Figure S2 consists of two subplots. Subplot (a) is a 'Cartoon illustration' showing a low-loss tunnel (black line) between two optima (Optima 1 and Optima 2) via a manifold. The tunnel is shown as a curved path. Subplot (b) shows 'Low-loss tunnel' metrics: Loss, Accuracy, and Label difference. The x-axis is 'Interpolation', ranging from 0.0 to 1.0. The y-axis for Loss ranges from 0 to 8. The y-axis for Accuracy ranges from 0.1 to 0.9. The y-axis for Label difference ranges from 0.00 to 0.35. The legend indicates: train on tunnel (orange solid), train on linear (orange dashed), val on tunnel (yellow solid), val on linear (yellow dashed), and label difference (black dashed).

Figure S2: (a) Cartoon illustration of a low-loss tunnel between two optima. (b) Loss, accuracy, and label difference along the tunnel.

Figure S2: *Left*: Cartoon illustration showing linear connector (black) along with the optimized connector which lies on the manifold of low loss solutions. *Right*: The loss and accuracy in between two independent optima on a linear path and an optimized path in the weight space.

In order to visualize the 2-dimensional cut through the loss landscape and the associated predictions along a curved low-loss path, we divide the path into linear segments, and compute the loss and prediction similarities on a triangle given by this segment on one side and the origin of the weight space on the other. We perform this operation on each of the linear segments from which the low-loss path is constructed, and place them next to each other for visualization. Figure S3 visualizes the loss along the manifold, as well as the similarity to the original optima. Note that the regions between radial yellow lines consist of segments, and we stitch these segments together in Figure S3. The accuracy plots show that as we traverse along the low-loss tunnel, the accuracy remains fairly constant as expected. However, the prediction similarity plot shows that the low-loss tunnel does not correspond to similar solutions in function space. What it shows is that while the modes are

![Figure S3: Radial loss landscape cut between the origin and two independent optima. (a) Accuracy along the low-loss tunnel, showing training and validation set accuracy. (b) Prediction similarity along the low-loss tunnel, showing similarity to optimum 1 and optimum 2.](36ac3e730a00d3f42d3400f5709f641a_img.jpg)

Figure S3 consists of two subplots, (a) and (b), showing radial loss landscape cuts. Subplot (a) displays 'Training set accuracy' and 'Validation set accuracy' on the y-axis (ranging from 0.38 to 0.52) against 'Towards optimum 1' and 'Towards optimum 2' on the x-axis (ranging from -1.0 to 1.0). A color bar indicates accuracy levels. Subplot (b) displays 'Similarity to optimum 1' and 'Similarity to optimum 2' on the y-axis (ranging from 0.00 to 1.00) against the same x-axis. A color bar indicates similarity levels. Both plots show a green shaded region representing the loss landscape, with a radial cut through the origin and towards two independent optima.

Figure S3: Radial loss landscape cut between the origin and two independent optima. (a) Accuracy along the low-loss tunnel, showing training and validation set accuracy. (b) Prediction similarity along the low-loss tunnel, showing similarity to optimum 1 and optimum 2.
...
connected in terms of accuracy/loss, their functional forms remain distinct and they do not collapse into a single mode.
...
![Figure S4: The effect of random initializations and random training batches on the diversity of predictions. The figure contains two plots, one for GPU and one for TPU, showing the fraction of test labels different versus learning rate for four different initialization and batching strategies.](602ada2a012ff3cc38d91de2eec5b450_img.jpg)

Figure S4 contains two plots, one for GPU and one for TPU, showing the 'Fraction of test labels different' on the y-axis (ranging from 0.0 to 0.5) versus 'Learning rate' on the x-axis (ranging from  $4.0 \times 10^{-4}$  to  $16.0 \times 10^{-4}$ ). The legend for both plots is:

- Random inits, random batches (blue)
- Random inits, fixed batches (orange)
- Fixed inits, random batches (green)
- Fixed inits, fixed batches (red)

On the GPU, the 'Fixed inits, fixed batches' (red) strategy shows a fraction of test labels different that is non-zero (around 0.05) at the lowest learning rate and increases slightly at higher learning rates. On the TPU, the 'Fixed inits, fixed batches' (red) strategy shows a fraction of test labels different that is zero at all learning rates.

Figure S4: The effect of random initializations and random training batches on the diversity of predictions. The figure contains two plots, one for GPU and one for TPU, showing the fraction of test labels different versus learning rate for four different initialization and batching strategies.
...
Zhang et al. [2020] show that vanilla stochastic gradient Markov Chain Monte Carlo (SGMCMC) methods do not explore multiple modes in the posterior and instead propose cyclic stochastic gradient MCMC (cSG-MCMC) to achieve that. We ran a suite of verification experiments to determine whether the diversity of functions found using the proposed cSG-MCMC algorithm matches that of independently randomly initialized and trained models.

We used the code published by the authors Zhang et al. [2020]<sup>5</sup> to match exactly the setup of their paper. We ran cSG-MCMC from 3 random initializations, each for a total of 150 epochs amounting to 3 cycles of the 50 epoch period learning rate schedule. We used a ResNet-18 and ran experiments on both CIFAR-10 and CIFAR-100. We measured the function diversity between a) independently initialized and trained runs, and b) between different cyclic learning rate periods within the same run of the cSG-MCMC. The latter (b) should be comparable to the former (a) if cSG-MCMC was as successful as vanilla deep ensembles at producing diverse functions. We show that both for CIFAR-10 and CIFAR-100, vanilla ensembles generate statistically significantly more diverse sets of functions than cSG-MCMC, as shown in Figure C. While cSG-MCMC is doing well in absolute terms, the shared initialization for cSG-MCMC training seems to lead to lower diversity than deep ensembles with multiple random initializations. Another difference between the methods is that individual members of deep ensemble can be trained in parallel unlike cSG-MCMC.

![Figure S5: Comparison of function space diversities between the cSG-MCMC (blue) and deep ensembles (red). The figure contains two box plots. The left plot is titled 'ResNet18 on CIFAR-10' and the right plot is titled 'ResNet18 on CIFAR-100'. Both plots show 'Normalized diversity' on the y-axis. In both cases, the 'Ensemble' (red) has a higher median diversity and a larger interquartile range compared to 'cSG-MCMC' (blue). Individual data points are overlaid on the box plots.](98ee20ceb85cd84e2415b20b1eda1bcf_img.jpg)

Figure S5: Comparison of function space diversities between the cSG-MCMC (blue) and deep ensembles (red). The figure contains two box plots. The left plot is titled 'ResNet18 on CIFAR-10' and the right plot is titled 'ResNet18 on CIFAR-100'. Both plots show 'Normalized diversity' on the y-axis. In both cases, the 'Ensemble' (red) has a higher median diversity and a larger interquartile range compared to 'cSG-MCMC' (blue). Individual data points are overlaid on the box plots.
...
In our diversity–accuracy plots (e.g. Figure 6), subspace samples trade off their accuracy for diversity in a characteristic way. To better understand where this relationship comes from, we derive several limiting curves based on an idealized model. We also propose a 1-parameter family of functions that provide a surprisingly good fit (given the simplicity of the model) to our observation, as shown in Figure S6.
...
<sup>5</sup> <https://github.com/ruqizhang/csgmcmc>

Only case 2 contributes to the fraction of labels on which they disagree. Hence we end up with

$$d_{\text{diff}}(a; a^*, C) = (1 - a^*)a + (1 - a)a^* + (1 - a^*)(1 - a)\frac{C - 2}{C - 1}.$$

This curve corresponds to the upper limit in Figure 6. The diversity reached in practice is not as high as the theoretical optimum even for the independently initialized and optimized solutions, which provides scope for future work.

#### D.2 Correlated predictions: the lower limit

By inspecting Figure 6 as well as a priori, we would expect a function  $f$  close to the reference function  $f^*$  in the weight space to have correlated predictions. We can model this by imagining that the predictions of  $f$  are just the predictions of the reference solution  $f^*$  perturbed by perturbations of a particular strength (which we vary). Let the probability of a label changing be  $p$ . We will consider four cases:
...
$$d_{\text{diff}}(a; a^*, C) = \frac{(C - 1)(a^* - a)}{Ca^* - 1}. \quad (2)$$

This curve corresponds to the lower limit in Figure 6.

#### D.3 Correlated predictions: 1-parameter family

![Figure S6: Theoretical model of the accuracy-diversity trade-off and a comparison to ResNet20v1 on CIFAR-100. The figure consists of two panels. The left panel shows theoretical curves for a 1-parameter family of functions f(p, p^e) for various values of e (0.01, 0.2, 0.5, 1, 2, 10). The right panel shows real subspace samples for ResNet20v1 on CIFAR-100, with a best-fitting curve for e=0.22. Both panels plot Diversity (y-axis, 0.0 to 1.0) against Test accuracy (x-axis, 0.0 to 0.6).](70f5f44cb855bbb0c203c00187b2113e_img.jpg)

Figure S6: Theoretical model of the accuracy-diversity trade-off and a comparison to ResNet20v1 on CIFAR-100. The figure consists of two panels. The left panel shows theoretical curves for a 1-parameter family of functions f(p, p^e) for various values of e (0.01, 0.2, 0.5, 1, 2, 10). The right panel shows real subspace samples for ResNet20v1 on CIFAR-100, with a best-fitting curve for e=0.22. Both panels plot Diversity (y-axis, 0.0 to 1.0) against Test accuracy (x-axis, 0.0 to 0.6).
...
We can improve upon this model by considering two separate probabilities of labels flipping:  $p_+$ , which is the probability that a correctly labelled example will flip, and  $p_-$ , corresponding to the probability that a wrongly labelled example will flip its label. By repeating the previous analysis, we obtain

$$d_{\text{diff}}(p_+, p_-; a^*, C) = a^*(1 - p_+) + (1 - a^*)p_- \frac{1}{C - 1}, \quad (3)$$

and

$$a(p_+, p_-; a^*, C) = a^* p_+ + (1 - a^*) p_- . \quad (4)$$

The previously derived lower limit corresponds to the case  $p_+ = p_- = p \in [0, 1]$ , where the probability of flipping the correct labels is the same as the probability of flipping the incorrect labels. The absolute worst case scenario would correspond to the situation where  $p_+ = p$ , while  $p_- = 0$ , i.e. only the correctly labelled examples flip.

We found that tying the two probabilities together via an exponent  $e$  as  $p_+ = p$  and  $p_- = p^e = p_+^e$  generates a realistic looking trade off between accuracy and diversity. We show the resulting functions for several values of  $e$  in Figure S6. For  $e < 1$ , the chance of flipping the wrong label is *larger* than that of the correct label, simulating a robustness of the learned solution to perturbations. We found the closest match to our data provided by  $e = 0.22$ .