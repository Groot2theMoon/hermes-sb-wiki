---
ingested: 2026-04-30
sha256: f2d2c6908acc9d861421ac2ab27da6f7d6977d62ea30335ae4b158f3836135ff
---
...
Zhenda Xie<sup>\*†</sup>, Yixuan Wei<sup>\*</sup>, Huanqi Cao<sup>\*</sup>,  
 ...
 Kui Yu, Liang Zhao, Shangyan Zhou, Zhean Xu, Zhengyan Zhang, Wangding Zeng,  
...
![Figure 1: Illustrations of Residual Connection Paradigms. (a) Residual Connection: Input x_i is processed by Layer F to produce h_i^out. A skip connection adds x_i to h_i^out via element-wise addition (circle with plus) to produce x_{i+1}. (b) Hyper-Connections (HC): Input x_i is processed by Res Mapping (H_i^{res}), Pre Mapping (H_i^{pre}), Layer F, and Post Mapping (H_i^{post}) to produce h_i^{post}. A skip connection adds x_i to h_i^{post} via element-wise addition to produce x_{i+1}. (c) Manifold-Constrained HC (mHC): Similar to (b), but the mapping blocks are constrained to a manifold, labeled with P_{M^{res}}(H_i^{res}), P_{M^{pre}}(H_i^{pre}), and P_{M^{post}}(H_i^{post}).](c2ddad56c1553f2499aaf7ef8cef6777_img.jpg)

(a) Residual Connection
(b) Hyper-Connections (HC)
(c) Manifold-Constrained HC (*m*HC)

Figure 1: Illustrations of Residual Connection Paradigms. (a) Residual Connection: Input x\_i is processed by Layer F to produce h\_i^out. A skip connection adds x\_i to h\_i^out via element-wise addition (circle with plus) to produce x\_{i+1}. (b) Hyper-Connections (HC): Input x\_i is processed by Res Mapping (H\_i^{res}), Pre Mapping (H\_i^{pre}), Layer F, and Post Mapping (H\_i^{post}) to produce h\_i^{post}. A skip connection adds x\_i to h\_i^{post} via element-wise addition to produce x\_{i+1}. (c) Manifold-Constrained HC (mHC): Similar to (b), but the mapping blocks are constrained to a manifold, labeled with P\_{M^{res}}(H\_i^{res}), P\_{M^{pre}}(H\_i^{pre}), and P\_{M^{post}}(H\_i^{post}).
...
![Figure 4: Communication-Computation Overlapping for mHC. The diagram shows three horizontal timelines: 'Normal Compute Stream', 'Communication Stream', and 'High Priority Compute Stream'. The Normal Compute Stream contains blocks for MLP (B), MLP (W), MLP (F), ATTN (B), ATTN (W), Whole Stage Recompute (B), and ATTN (F). The Communication Stream contains blocks for DISPATCH (F), DISPATCH (B), COMBINE (F), PP Send Recv (F), PP Send Recv (B), and COMBINE (B). The High Priority Compute Stream shows the overlap of these streams, with labels for F^M_{pilot, res} (F) and F^M_{pilot, res} (B) indicating the start and end of the high-priority stream.](d4af765160d04ecef538e5066006dc77_img.jpg)

Figure 4: Communication-Computation Overlapping for mHC. The diagram shows three horizontal timelines: 'Normal Compute Stream', 'Communication Stream', and 'High Priority Compute Stream'. The Normal Compute Stream contains blocks for MLP (B), MLP (W), MLP (F), ATTN (B), ATTN (W), Whole Stage Recompute (B), and ATTN (F). The Communication Stream contains blocks for DISPATCH (F), DISPATCH (B), COMBINE (F), PP Send Recv (F), PP Send Recv (B), and COMBINE (B). The High Priority Compute Stream shows the overlap of these streams, with labels for F^M\_{pilot, res} (F) and F^M\_{pilot, res} (B) indicating the start and end of the high-priority stream.
...
![Figure 5: Training Stability of Manifold-Constrained Hyper-Connections (mHC). (a) Absolute Training Loss Gap vs. Training Steps: A line graph showing the absolute loss gap for Baseline, HC, and mHC over 50,000 steps. The Baseline and HC lines are relatively flat near zero, while the mHC line shows a significant negative gap, reaching approximately -0.04. (b) Gradient Norm vs. Training Steps: A line graph showing the gradient norm for Baseline, HC, and mHC over 50,000 steps. The Baseline and mHC lines are relatively stable around 0.1, while the HC line shows significant fluctuations and spikes, reaching up to 0.20.](dd330f8b8f6c16eae20c3a676b4eb804_img.jpg)

Figure 5: Training Stability of Manifold-Constrained Hyper-Connections (mHC). (a) Absolute Training Loss Gap vs. Training Steps: A line graph showing the absolute loss gap for Baseline, HC, and mHC over 50,000 steps. The Baseline and HC lines are relatively flat near zero, while the mHC line shows a significant negative gap, reaching approximately -0.04. (b) Gradient Norm vs. Training Steps: A line graph showing the gradient norm for Baseline, HC, and mHC over 50,000 steps. The Baseline and mHC lines are relatively stable around 0.1, while the HC line shows significant fluctuations and spikes, reaching up to 0.20.
...
Figure 6 consists of two subplots, (a) and (b), each containing two y-axes. The left y-axis represents 'Absolute Loss Gap' and the right y-axis represents 'Relative Loss Ratio'.

(a) Compute Scaling Curve: The x-axis is 'FLOPs' on a logarithmic scale from  $10^{21}$  to  $10^{22}$ . The left y-axis for Absolute Loss Gap ranges from -0.04 to 0.02. The right y-axis for Relative Loss Ratio ranges from 98.0% to 101.0%. The Baseline is represented by a solid line at 0.00 on the left axis and 100.0% on the right axis. The *mHC* data points (circles) show a negative Absolute Loss Gap (around -0.03) and a Relative Loss Ratio slightly below 100.0% (around 98.5%).

(b) Token Scaling Curve: The x-axis is 'FLOPs' on a linear scale from 2 to 4, with a multiplier of  $\times 10^{21}$ . The left y-axis for Absolute Loss Gap ranges from -0.03 to 0.01. The right y-axis for Relative Loss Ratio ranges from 98.0% to 101.0%. The Baseline is represented by a solid line at 0.00 on the left axis and 100.0% on the right axis. The *mHC* data points (circles) show a negative Absolute Loss Gap (around -0.02) and a Relative Loss Ratio slightly below 100.0% (around 98.5%).
...
![Figure 7: Propagation Stability of Manifold-Constrained Hyper-Connections (mHC). (a) Single-Layer Mapping: A line graph showing Amax Gain Magnitude (0.0 to 2.0) vs Layer Index l (0 to 60). The Forward Signal Gain (grey line) and Backward Gradient Gain (blue line) both remain nearly constant at 1.0. (b) Composite Mapping: A line graph showing Amax Gain Magnitude (0.0 to 2.0) vs Layer Index l (0 to 60). The Forward Signal Gain (grey line) starts at 1.0, peaks at approximately 1.6 around layer 20, and then gradually decreases to 1.0 by layer 60. The Backward Gradient Gain (blue line) starts at 1.0, peaks at approximately 1.6 around layer 20, and then gradually decreases to 1.0 by layer 60.](7c1f9e78e0f033d391b687f1652f6e47_img.jpg)

Figure 7: Propagation Stability of Manifold-Constrained Hyper-Connections (mHC). (a) Single-Layer Mapping: A line graph showing Amax Gain Magnitude (0.0 to 2.0) vs Layer Index l (0 to 60). The Forward Signal Gain (grey line) and Backward Gradient Gain (blue line) both remain nearly constant at 1.0. (b) Composite Mapping: A line graph showing Amax Gain Magnitude (0.0 to 2.0) vs Layer Index l (0 to 60). The Forward Signal Gain (grey line) starts at 1.0, peaks at approximately 1.6 around layer 20, and then gradually decreases to 1.0 by layer 60. The Backward Gradient Gain (blue line) starts at 1.0, peaks at approximately 1.6 around layer 20, and then gradually decreases to 1.0 by layer 60.

Figure 7 | **Propagation Stability of Manifold-Constrained Hyper-Connections (mHC).** This figure illustrates the propagation dynamics of (a) the single-layer mapping  $\mathcal{P}_{\mathcal{M}^{\text{res}}}(\mathcal{H}_l^{\text{res}})$  and (b) the composite mapping  $\prod_{i=l}^{l-1} \mathcal{P}_{\mathcal{M}^{\text{res}}}(\mathcal{H}_{i-l}^{\text{res}})$  within the 27B model. The results demonstrate that mHC significantly enhances propagation stability compared to HC.

![Figure 8: Visualizations of Learnable Mappings. This figure displays representative single-layer and composite mappings for HC (first row) and mHC (second row). Each matrix is computed by averaging over all tokens within a selected sequence. The labels annotated along the y-axis and x-axis indicate the forward signal gain (row sum) and the backward gradient gain (column sum), respectively. The figure shows 6x6 matrices for HC and mHC, with row and column sums indicating the gain magnitudes.](c2b98986bdf45e15707f6b2bd7ade2bd_img.jpg)

Figure 8: Visualizations of Learnable Mappings. This figure displays representative single-layer and composite mappings for HC (first row) and mHC (second row). Each matrix is computed by averaging over all tokens within a selected sequence. The labels annotated along the y-axis and x-axis indicate the forward signal gain (row sum) and the backward gradient gain (column sum), respectively. The figure shows 6x6 matrices for HC and mHC, with row and column sums indicating the gain magnitudes.
