---
ingested: 2026-04-30
sha256: 40f8aad2d2f6d58be2f728a115462cfad80701671509cf2042866b68db4b7a01
---
...
![Figure 1: Training error (left) and test error (right) on CIFAR-10 with 20-layer and 56-layer 'plain' networks. The left plot shows training error (%) decreasing over iterations (1e4) for a 56-layer network (red line) and a 20-layer network (green line). The 56-layer network has a higher training error than the 20-layer network. The right plot shows test error (%) decreasing over iterations (1e4) for the same two networks. The 56-layer network has a higher test error than the 20-layer network.](59c76d920f3faf3933d80a47647d42a9_img.jpg)
...
![Diagram of a residual learning building block. The input x is processed through a 'weight layer' and a 'relu' layer to produce F(x). A skip connection branches off from the input x, labeled 'identity', and bypasses the main layers. The output of the main layers, F(x), is then added to the identity output at a block labeled with a '+' sign. The result of the addition is passed through another 'relu' layer to produce the final output F(x) + x.](68ac34ff111db52afaa786afcb8346c3_img.jpg)

Diagram of a residual learning building block. The input x is processed through a 'weight layer' and a 'relu' layer to produce F(x). A skip connection branches off from the input x, labeled 'identity', and bypasses the main layers. The output of the main layers, F(x), is then added to the identity output at a block labeled with a '+' sign. The result of the addition is passed through another 'relu' layer to produce the final output F(x) + x.
...
**Shortcut Connections.** Practices and theories that lead to shortcut connections [2, 34, 49] have been studied for a long time. An early practice of training multi-layer perceptrons (MLPs) is to add a linear layer connected from the network input to the output [34, 49]. In [44, 24], a few intermediate layers are directly connected to auxiliary classifiers for addressing vanishing/exploding gradients. The papers of [39, 38, 31, 47] propose methods for centering layer responses, gradients, and propagated errors, implemented by shortcut connections. In [44], an “inception” layer is composed of a shortcut branch and a deep branch.
...
| conv2_x    | 56×56       | [3×3, 64<br>3×3, 64] ×2          | [3×3, 64<br>3×3, 64] ×3   | [1×1, 64<br>3×3, 64<br>1×1, 256] ×3    | [1×1, 64<br>3×3, 64<br>1×1, 256] ×3     | [1×1, 64<br>3×3, 64<br>1×1, 256] ×3     |
| conv3_x    | 28×28       | [3×3, 128<br>3×3, 128] ×2        | [3×3, 128<br>3×3, 128] ×4 | [1×1, 128<br>3×3, 128<br>1×1, 512] ×4  | [1×1, 128<br>3×3, 128<br>1×1, 512] ×4   | [1×1, 128<br>3×3, 128<br>1×1, 512] ×8   |
| conv4_x    | 14×14       | [3×3, 256<br>3×3, 256] ×2        | [3×3, 256<br>3×3, 256] ×6 | [1×1, 256<br>3×3, 256<br>1×1, 1024] ×6 | [1×1, 256<br>3×3, 256<br>1×1, 1024] ×23 | [1×1, 256<br>3×3, 256<br>1×1, 1024] ×36 |
| conv5_x    | 7×7         | [3×3, 512<br>3×3, 512] ×2        | [3×3, 512<br>3×3, 512] ×3 | [1×1, 512<br>3×3, 512<br>1×1, 2048] ×3 | [1×1, 512<br>3×3, 512<br>1×1, 2048] ×3  | [1×1, 512<br>3×3, 512<br>1×1, 2048] ×3  |
...
![Figure 4: Training on ImageNet. Two plots showing error (%) vs iteration (iter. (1e4)). Left plot: plain networks of 18 and 34 layers. Right plot: ResNets of 18 and 34 layers. In both plots, thin lines represent training error and bold lines represent validation error. In the left plot, the 34-layer plain network (red) has higher error than the 18-layer plain network (cyan). In the right plot, the 34-layer ResNet (red) has lower error than the 18-layer ResNet (cyan).](7a3561af571faf036baa93f5f4b1bdb9_img.jpg)

Figure 4: Training on ImageNet. Two plots showing error (%) vs iteration (iter. (1e4)). Left plot: plain networks of 18 and 34 layers. Right plot: ResNets of 18 and 34 layers. In both plots, thin lines represent training error and bold lines represent validation error. In the left plot, the 34-layer plain network (red) has higher error than the 18-layer plain network (cyan). In the right plot, the 34-layer ResNet (red) has lower error than the 18-layer ResNet (cyan).
...
![Figure 5: Diagrams of building blocks for ResNet. Left: A building block for ResNet-34 with 64-d input/output. It consists of two 3x3, 64 layers with a ReLU activation between them. A skip connection bypasses both layers and is added to the output of the second layer via a ReLU activation. Right: A 'bottleneck' building block for ResNet-50/101/152 with 256-d input/output. It consists of three layers: 1x1, 64; 3x3, 64; and 1x1, 256, each followed by a ReLU. A skip connection bypasses all three layers and is added to the output of the third layer via a ReLU activation.](5d76c2e9976aaf97001ee189908830fb_img.jpg)
...
![Figure 6: Training on CIFAR-10. Three plots showing error (%) vs. iter (1e4). Left: plain networks (plain-20, 32, 44, 56). Middle: ResNets (ResNet-20, 32, 44, 56, 101). Right: ResNets with 110 and 1202 layers (residual-110, residual-1202). Dashed lines are training error, bold lines are testing error. Plain-110 error is >60%.](c3c305cefbac2e7b13be34ab87054d1e_img.jpg)

Figure 6: Training on CIFAR-10. Three plots showing error (%) vs. iter (1e4). Left: plain networks (plain-20, 32, 44, 56). Middle: ResNets (ResNet-20, 32, 44, 56, 101). Right: ResNets with 110 and 1202 layers (residual-110, residual-1202). Dashed lines are training error, bold lines are testing error. Plain-110 error is >60%.

Figure 6. Training on **CIFAR-10**. Dashed lines denote training error, and bold lines denote testing error. **Left:** plain networks. The error of plain-110 is higher than 60% and not displayed. **Middle:** ResNets. **Right:** ResNets with 110 and 1202 layers.

![Figure 7: Standard deviations (std) of layer responses on CIFAR-10. Two plots vs. layer index. Top: original order. Bottom: sorted by magnitude. Legend: plain-20, plain-56, ResNet-20, ResNet-56, ResNet-110.](a71911ad87414271aeb190e0eebcb989_img.jpg)

Figure 7: Standard deviations (std) of layer responses on CIFAR-10. Two plots vs. layer index. Top: original order. Bottom: sorted by magnitude. Legend: plain-20, plain-56, ResNet-20, ResNet-56, ResNet-110.

Figure 7. Standard deviations (std) of layer responses on **CIFAR-10**. The responses are the outputs of each  $3 \times 3$  layer, after BN and before nonlinearity (ReLU/addition). For ResNets, this analysis reveals the response strength of the residual functions. Fig. 7 shows that ResNets have generally smaller responses than their plain counterparts. These results support our basic motivation (Sec.3.1) that the residual functions might be generally closer to zero than the non-residual functions. We also notice that the deeper ResNet has smaller magnitudes of responses, as evidenced by the comparisons among ResNet-20, 56, and 110 in Fig. 7. When there are more layers, an individual layer of ResNets tends to modify the signal less.
...
- [28] G. Montúfar, P. Pascanu, K. Cho, and Y. Bengio. On the number of linear regions of deep neural networks. In *NIPS*, 2014.
...
- [35] A. Romero, N. Ballas, S. E. Kahou, A. Chassang, C. Gatta, and Y. Bengio. Fitness: Hints for thin deep nets. In *ICLR*, 2015.
...
| system      | net        | data       | mAP         | aco         | bke         | bird        | boat        | bottle      | bus         | car         | cat         | chair       | cow         | table       | dog         | horse       | mbike       | person      | plant       | sheep       | sofa        | train       | tv          |
|-------------|------------|------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|-------------|
...
| baseline+++ | ResNet-101 | COCO+07+12 | <b>85.6</b> | <b>90.0</b> | <b>89.6</b> | <b>87.8</b> | <b>80.8</b> | <b>76.1</b> | <b>89.9</b> | <b>89.9</b> | <b>89.6</b> | <b>75.5</b> | <b>90.0</b> | <b>80.7</b> | <b>89.6</b> | <b>90.3</b> | <b>89.1</b> | <b>88.7</b> | <b>65.4</b> | <b>88.1</b> | <b>85.6</b> | <b>89.0</b> | <b>86.8</b> |
