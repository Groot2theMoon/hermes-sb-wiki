...
$$\begin{aligned} h_0 &= U W_e + W_p \\ h_i &= \text{transformer\_block}(h_{i-1}) \forall i \in [1, n] \\ P(u) &= \text{softmax}(h_n W_e^T) \end{aligned} \quad (2)$$
...
![Figure 1: (left) Transformer architecture and training objectives used in this work. (right) Input transformations for fine-tuning on different tasks. We convert all structured inputs into token sequences to be processed by our pre-trained model, followed by a linear+softmax layer.](2763901b7a1fd1b5d704cdc450d12ed0_img.jpg)

The diagram illustrates the Transformer architecture and input transformations for various tasks. On the left, the architecture shows a stack of 12 identical layers. Each layer contains a 'Masked Multi Self Attention' block, a 'Layer Norm' block, a 'Feed Forward' block, and another 'Layer Norm' block, with residual connections around each. The input is a 'Text & Position Embed' block. The output of the 12th layer is passed to a 'Text Prediction' and 'Task Classifier' block. On the right, input transformations for different tasks are shown:

- Classification:** [Start, Text, Extract] → Transformer → Linear
- Entailment:** [Start, Premise, Delim, Hypothesis, Extract] → Transformer → Linear
- Similarity:** Two sequences, [Start, Text 1, Delim, Text 2, Extract] and [Start, Text 2, Delim, Text 1, Extract], are processed by separate Transformers. Their outputs are added together (+) and then passed to a Linear layer.
- Multiple Choice:** Multiple sequences (e.g., [Start, Context, Delim, Answer 1, Extract], [Start, Context, Delim, Answer 2, Extract], ..., [Start, Context, Delim, Answer N, Extract]) are processed by separate Transformers. Their outputs are combined (indicated by a bracket and a small diagram of a vector) and then passed to a Linear layer.
...
**Model specifications** Our model largely follows the original transformer work [62]. We trained a 12-layer decoder-only transformer with masked self-attention heads (768 dimensional states and 12 attention heads). For the position-wise feed-forward networks, we used 3072 dimensional inner states. We used the Adam optimization scheme [27] with a max learning rate of  $2.5\text{e-}4$ . The learning rate was increased linearly from zero over the first 2000 updates and annealed to 0 using a cosine schedule. We train for 100 epochs on minibatches of 64 randomly sampled, contiguous sequences of 512 tokens. Since layernorm [2] is used extensively throughout the model, a simple weight initialization of  $N(0, 0.02)$  was sufficient. We used a bytepair encoding (BPE) vocabulary with 40,000 merges [53] and residual, embedding, and attention dropouts with a rate of 0.1 for regularization. We also employed a modified version of L2 regularization proposed in [37], with  $w = 0.01$  on all non bias or gain weights. For the activation function, we used the Gaussian Error Linear Unit (GELU) [18]. We used learned position embeddings instead of the sinusoidal version proposed in the original work. We use the *fifty* library<sup>2</sup> to clean the raw text in BooksCorpus, standardize some punctuation and whitespace, and use the *spaCy* tokenizer.<sup>3</sup>

**Fine-tuning details** Unless specified, we reuse the hyperparameter settings from unsupervised pre-training. We add dropout to the classifier with a rate of 0.1. For most tasks, we use a learning rate of  $6.25\text{e-}5$  and a batchsize of 32. Our model finetunes quickly and 3 epochs of training was sufficient for most cases. We use a linear learning rate decay schedule with warmup over 0.2% of training.  $\lambda$  was set to 0.5.
...
<sup>2</sup><https://ftfy.readthedocs.io/en/latest/>
...
|---------------------------------------|----------------|---------------|---------------------|---------------|-------------|-------------|
|                                       | CoLA<br>(mc)   | SST2<br>(acc) | MRPC<br>(F1)        | STS-B<br>(pc) | QQP<br>(F1) |             |
...
**Impact of number of layers transferred** We observed the impact of transferring a variable number of layers from unsupervised pre-training to the supervised target task. Figure 2(left) illustrates the performance of our approach on MultiNLQ and RACE as a function of the number of layers transferred. We observe the standard result that transferring embeddings improves performance and that each transformer layer provides further benefits up to 9% for full transfer on MultiNLQ. This indicates that each layer in the pre-trained model contains useful functionality for solving target tasks.

![Figure 2: (left) Effect of transferring increasing number of layers from the pre-trained language model on RACE and MultiNLQ. (right) Plot showing the evolution of zero-shot performance on different tasks as a function of LM pre-training updates.](4279c8be6ec4ed56f4b3349be98bb426_img.jpg)

Figure 2 consists of two line plots. The left plot shows 'RACE Dev Accuracy' (left y-axis, 30-100) and 'MultiNLQ Dev Accuracy' (right y-axis, 70-100) against the '# of layers transferred' (x-axis, 0-12). It includes four series: RACE Dev (solid blue), RACE Train (dashed blue), MultiNLQ Dev (solid orange), and MultiNLQ Train (dashed orange). All series show an upward trend as more layers are transferred, with MultiNLQ Train reaching the highest accuracy of approximately 98% at 12 layers. The right plot shows 'Relative Task Performance' (y-axis, 0.0-1.0) against the '# of pre-training updates' (x-axis, 10^3-10^6) on a logarithmic scale. It includes six series: sentiment analysis (solid blue), winograd schema resolution (solid orange), linguistic acceptability (solid green), question answering (solid red), Transformer (solid black), and LSTM (dashed black). Most tasks show an increase in relative performance over updates, with sentiment analysis reaching a plateau around 0.75 and question answering showing a slight dip after 10^5 updates.

Figure 2: (left) Effect of transferring increasing number of layers from the pre-trained language model on RACE and MultiNLQ. (right) Plot showing the evolution of zero-shot performance on different tasks as a function of LM pre-training updates.

Figure 2: (left) Effect of transferring increasing number of layers from the pre-trained language model on RACE and MultiNLQ. (right) Plot showing the evolution of zero-shot performance on different tasks as a function of LM pre-training updates. Performance per task is normalized between a random guess baseline and the current state-of-the-art with a single model.
...
| Method                       | Avg. Score  | CoLA<br>(mc) | SST2<br>(acc) | MRPC<br>(F1) | STS-B<br>(pc) | QQP<br>(F1) | MNLI<br>(acc) | QNLI<br>(acc) | RTE<br>(acc) |
|------------------------------|-------------|--------------|---------------|--------------|---------------|-------------|---------------|---------------|--------------|
...
- [23] Y. Ji and J. Eisenstein. Discriminative improvements to distributional sentence similarity. In *Proceedings of the Third Conference on Empirical Methods in Natural Language Processing*, pages 891–896, 2013.
