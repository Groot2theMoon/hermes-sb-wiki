---
ingested: 2026-04-30
sha256: e30ce9f7dbc2860f176242f82b18576402655df8c94638c97c4703db141794a7
---
...
This paper demonstrates an approach for learning highly semantic image representations without relying on hand-crafted data-augmentations. We introduce the Image-based Joint-Embedding Predictive Architecture (I-JEPA), a non-generative approach for self-supervised learning from images. The idea behind I-JEPA is simple: from a single context block in the same image, predict the representations of various target blocks in the same image. A core design choice to guide I-JEPA towards producing semantic representations is the masking strategy; specifically, it is crucial to (a) sample target blocks with sufficiently large scale (semantic), and to (b) use a sufficiently informative (spatially distributed) context block. Empirically, when combined with Vision Transformers, we find I-JEPA to be highly scalable. For instance, we train a ViT-Huge/14 on ImageNet using 16 A100 GPUs in under 72 hours to achieve strong downstream performance across a wide range of tasks, from linear classification to object counting and depth prediction.
...
![Figure 1: ImageNet-1K Linear Evaluation vs GPU Hours. A scatter plot showing Top 1 (%) performance on the y-axis (ranging from 76 to 82) against Pretraining GPU Hours on a logarithmic x-axis (ranging from 10^3 to 10^4). I-JEPA (ViT-H/14 500ep) is shown as a green dashed line with two points, starting at approximately 79.5% at 10^3 hours and reaching about 81.2% at 10^4 hours. Other methods are shown as black circles: iBOT (ViT-S/16 800ep) at ~77.5%, CAE (ViT-L/16 1600ep) at ~78.2%, data2vec (ViT-L/16 1600ep) at ~77.2%, and MAE (ViT-H/14 1600ep) at ~77.8%.](1066b22ea190b81c91c3441ea7618236_img.jpg)

| Method   | Model    | Epochs | GPU Hours (approx.)    | Top 1 (%) (approx.) |
|----------|----------|--------|------------------------|---------------------|
| I-JEPA   | ViT-H/14 | 500    | 10 <sup>3</sup>        | 79.5                |
| I-JEPA   | ViT-H/14 | 500    | 10 <sup>4</sup>        | 81.2                |
| iBOT     | ViT-S/16 | 800    | ~3.5 × 10 <sup>3</sup> | 77.5                |
| CAE      | ViT-L/16 | 1600   | ~8 × 10 <sup>3</sup>   | 78.2                |
| data2vec | ViT-L/16 | 1600   | ~9 × 10 <sup>3</sup>   | 77.2                |
| MAE      | ViT-H/14 | 1600   | ~10 <sup>4</sup>       | 77.8                |

Figure 1: ImageNet-1K Linear Evaluation vs GPU Hours. A scatter plot showing Top 1 (%) performance on the y-axis (ranging from 76 to 82) against Pretraining GPU Hours on a logarithmic x-axis (ranging from 10^3 to 10^4). I-JEPA (ViT-H/14 500ep) is shown as a green dashed line with two points, starting at approximately 79.5% at 10^3 hours and reaching about 81.2% at 10^4 hours. Other methods are shown as black circles: iBOT (ViT-S/16 800ep) at ~77.5%, CAE (ViT-L/16 1600ep) at ~78.2%, data2vec (ViT-L/16 1600ep) at ~77.2%, and MAE (ViT-H/14 1600ep) at ~77.8%.
...
![Diagram of the I-JEPA architecture. A 'context' image block is processed by a 'context encoder' (f_theta) to produce a representation. This representation is fed into a 'predictor' which consists of multiple blocks (g_phi) that predict the representations of various 'target' blocks. The 'target' image block is processed by a 'target encoder' (f_theta) to produce its representation. The predictor's output is compared to the target's representation using an L2 loss function.](2df09d249c38abee55dbf0526dd6da0a_img.jpg)

The diagram illustrates the I-JEPA architecture. On the left, a 'context' image block is processed by a 'context encoder' labeled  $f_\theta$ , which outputs a sequence of colored squares (blue, red, yellow). These are fed into a 'predictor' block labeled  $g_\phi$ . The predictor also receives positional tokens (shown as colored squares: blue, red, yellow) and outputs predicted representations for target blocks. On the right, a 'target' image block is processed by a 'target encoder' labeled  $f_\theta$ , which also outputs a sequence of colored squares. The predicted representations from the predictor are compared to the target representations using an  $L_2$  loss function, indicated by a dashed line and the label  $L_2$ .

Diagram of the I-JEPA architecture. A 'context' image block is processed by a 'context encoder' (f\_theta) to produce a representation. This representation is fed into a 'predictor' which consists of multiple blocks (g\_phi) that predict the representations of various 'target' blocks. The 'target' image block is processed by a 'target encoder' (f\_theta) to produce its representation. The predictor's output is compared to the target's representation using an L2 loss function.
...
![Figure 4: A grid of images illustrating the context and target-masking strategy. The grid has 4 rows and 6 columns. The first column is labeled 'original' and shows the full image. The second column is labeled 'context' and shows a single context block. The remaining four columns are labeled 'targets' and show four different target blocks. The images include a dog, a cat, a person, and a landscape.](5f18c728fc511750ffcaa626716b920e_img.jpg)

Figure 4: A grid of images illustrating the context and target-masking strategy. The grid has 4 rows and 6 columns. The first column is labeled 'original' and shows the full image. The second column is labeled 'context' and shows a single context block. The remaining four columns are labeled 'targets' and show four different target blocks. The images include a dog, a cat, a person, and a landscape.

Figure 4. Examples of our context and target-masking strategy. Given an image, we randomly sample 4 target blocks with scale in the range  $(0.15, 0.2)$  and aspect ratio in the range  $(0.75, 1.5)$ . Next, we randomly sample a context block with scale in the range  $(0.85, 1.0)$  and unit aspect ratio. We remove any overlapping target blocks. Under this strategy, the target-blocks are relatively semantic, and the context-block is informative, yet sparse (efficient to process).
...
![Figure 5: Scaling. Semi-supervised evaluation on ImageNet-1K 1% as a function of pretraining GPU hours. The plot shows Top 1 (%) on the y-axis (60 to 80) and Pretraining GPU Hours on a logarithmic x-axis (10^3 to 10^4). Data points for I-JEPA (circles), iBOT (triangles), MAE (squares), and data2vec (diamonds) are shown. I-JEPA points are labeled with model/architecture and epochs: ViT-B/16 (1000ep), ViT-H/14 (300ep), ViT-H/16 (600ep), ViT-H/16 (1000ep), ViT-L/16 (1000ep), and ViT-H/14 (1000ep).](c0843c6d138705289960d9f53a6e72a1_img.jpg)

Figure 5: Scaling. Semi-supervised evaluation on ImageNet-1K 1% as a function of pretraining GPU hours. The plot shows Top 1 (%) on the y-axis (60 to 80) and Pretraining GPU Hours on a logarithmic x-axis (10^3 to 10^4). Data points for I-JEPA (circles), iBOT (triangles), MAE (squares), and data2vec (diamonds) are shown. I-JEPA points are labeled with model/architecture and epochs: ViT-B/16 (1000ep), ViT-H/14 (300ep), ViT-H/16 (600ep), ViT-H/16 (1000ep), ViT-L/16 (1000ep), and ViT-H/14 (1000ep).
...
![Figure 6: Visualization of I-JEPA predictor representations. The figure shows a grid of images. Each row represents a different object: a bird, a hamburger, an alarm clock, and a group of fruits. The first column shows the original image. The second column shows the context image with a green bounding box. The subsequent columns show samples from a generative model decoding the output of the pretrained I-JEPA predictor, with green bounding boxes indicating the location of the predicted samples. The samples show high-level object parts (e.g., the back of the bird, the top of a car) but discard precise low-level details and background information.](c834b9abb4ddf70e5d10641f87d5ff5b_img.jpg)
...
- [52] Yi Ma, Doris Tsao, and Heung-Young Shum. On the principles of parsimony and self-consistency for the emergence of intelligence. *Frontiers of Information Technology & Electronic Engineering*, pages 1–26, 2022. 13
...
|--------------|----------|--------|-------------|----------------------|
| 0.04 → 0.4   | ViT-L/16 | 600    | 69.4        | <b>77.8</b>          |
...
![Figure 7: Visualization of I-JEPA target-encoder representations. The figure is a 4x5 grid of images. The first column shows the original images: a black and white bird on a branch, a vintage television set, a pink daisy-like flower, and a small brown dog in a field. The subsequent four columns show generated samples for each row. The bird samples maintain the black and white pattern but vary in pose and background. The television samples all show a screen with a single bright white circle, losing details of the TV's design. The flower samples all have a similar pink color and radial pattern but differ in the center's texture. The dog samples all show a small, fluffy brown dog in a grassy field, maintaining the general appearance but with some variation in pose and background details.](ac852a162572ca8a8c8478c49b571af5_img.jpg)

Figure 7: Visualization of I-JEPA target-encoder representations. The figure is a 4x5 grid of images. The first column shows the original images: a black and white bird on a branch, a vintage television set, a pink daisy-like flower, and a small brown dog in a field. The subsequent four columns show generated samples for each row. The bird samples maintain the black and white pattern but vary in pose and background. The television samples all show a screen with a single bright white circle, losing details of the TV's design. The flower samples all have a similar pink color and radial pattern but differ in the center's texture. The dog samples all show a small, fluffy brown dog in a grassy field, maintaining the general appearance but with some variation in pose and background details.
...
![Figure 8: Visualization of MSN target-encoder representations. The figure is a 4x5 grid of images. The first column shows the original images: a yellow fish, a brown chicken, a peacock, and a koala. The subsequent four columns show generated samples for each row. The fish samples are very blurry and vary significantly in color and shape. The chicken samples all appear as a single, somewhat blurry brown chicken. The peacock samples all show a peacock with its tail feathers fanned out, but the colors and patterns vary. The koala samples all show a koala eating eucalyptus leaves, but the background and the koala's pose vary significantly.](93afce28d7dec5b2202789b31b4ef8ab_img.jpg)

Figure 8: Visualization of MSN target-encoder representations. The figure is a 4x5 grid of images. The first column shows the original images: a yellow fish, a brown chicken, a peacock, and a koala. The subsequent four columns show generated samples for each row. The fish samples are very blurry and vary significantly in color and shape. The chicken samples all appear as a single, somewhat blurry brown chicken. The peacock samples all show a peacock with its tail feathers fanned out, but the colors and patterns vary. The koala samples all show a koala eating eucalyptus leaves, but the background and the koala's pose vary significantly.
