---
ingested: 2026-04-30
sha256: 7b88e00ff6c69e0c9498a47c05fe94eeee79f4fdc8a3471ccf552a77c4d49b60
---
...
$$\sum_{k=1}^N ( \log p(t_k | t_1, \dots, t_{k-1}; \Theta_x, \vec{\mathbf{O}}_{LSTM}, \Theta_s) + \log p(t_k | t_{k+1}, \dots, t_N; \Theta_x, \overleftarrow{\mathbf{O}}_{LSTM}, \Theta_s) ).$$
...
![Figure 1: Comparison of baseline vs. ELMo performance for SNLI and SRL. Two line graphs show accuracy and F1 score respectively, across training set sizes from 0.1% to 100%.](7ff005f9556dc6518981bb92091d36ab_img.jpg)

Figure 1 consists of two line graphs. The left graph is titled 'SNLI (Accuracy)' and the right graph is titled 'SRL (F1)'. Both graphs plot performance against training set size on a logarithmic x-axis with values 0.1%, 1%, 10%, and 100%. Two lines are shown: a solid line for 'With ELMo' and a dashed line for 'Baseline'. In the SNLI graph, the ELMo line starts at ~60% and ends at ~88%, while the Baseline line starts at ~48% and ends at ~86%. Improvements are labeled as +12.3, +7.7, +1.5, and +1.4. In the SRL graph, the ELMo line starts at ~38% and ends at ~85%, while the Baseline line starts at ~20% and ends at ~82%. Improvements are labeled as +18.6, +20.5, +10.8, and +3.1.

Figure 1: Comparison of baseline vs. ELMo performance for SNLI and SRL. Two line graphs show accuracy and F1 score respectively, across training set sizes from 0.1% to 100%.

Figure 1: Comparison of baseline vs. ELMo performance for SNLI and SRL as the training set size is varied from 0.1% to 100%.

![Figure 2: Visualization of softmax normalized biLM layer weights across tasks and ELMo locations. A heatmap shows weights for LSTM 2, LSTM 1, and Token layers across various tasks.](aa14b9ec884bf40ce06c161be468cd84_img.jpg)

Figure 2 is a heatmap showing softmax-normalized biLM layer weights. The y-axis lists 'LSTM 2', 'LSTM 1', and 'Token'. The x-axis is divided into 'Input Layer' (SRL, Coref, SNLI, SQuAD, SST-5) and 'Output Layer' (SNLI, SQuAD, SST-5). A color scale on the right ranges from 0 (black) to 1 (white), with intermediate markers at 1/6, 1/3, 1/2, 2/3, and 5/6. Horizontal hatching indicates weights less than 1/3, and speckling indicates weights greater than 2/3. The heatmap shows that for the input layer, the first biLSTM layer (LSTM 1) is strongly favored for coreference and SQuAD, while the output layer weights are more balanced.

Figure 2: Visualization of softmax normalized biLM layer weights across tasks and ELMo locations. A heatmap shows weights for LSTM 2, LSTM 1, and Token layers across various tasks.
...
- Tsendsuren Munkhdalai and Hong Yu. 2017. Neural tree indexes for text understanding. In *EACL*.
...
### A.2 Importance of $\gamma$ in Eqn. (1)

The  $\gamma$  parameter in Eqn. (1) was of practical importance to aid optimization, due to the different distributions between the biLM internal representations and the task specific representations. It is especially important in the last-only case in Sec. 5.1. Without this parameter, the last-only case performed poorly (well below the baseline) for SNLI and training failed completely for SRL.
...
The best ELMo configuration added ELMo vectors to both the input and output of the lowest layer LSTM, using (1) with layer normalization and  $\lambda = 0.001$ . Due to the increased number of parameters in the ELMo model, we added  $\ell^2$  regularization with regularization coefficient 0.0001 to all recurrent and feed forward weight matrices and 50% dropout after the attention layer.
...
<sup>3</sup>A comprehensive comparison can be found at <https://nlp.stanford.edu/projects/snli/>
...
Table 9: Test set results for SQuAD, showing both Exact Match (EM) and F<sub>1</sub>. The top half of the table contains single model results with ensembles at the bottom. References provided where available.
...
Table 11 compares our results with previously published results. Overall, we improve the single model state-of-the-art by 3.2% average F<sub>1</sub>, and our single model result improves the previous ensemble best by 1.6% F<sub>1</sub>. Adding ELMo to the output from the biLSTM in addition to the biLSTM input reduced F<sub>1</sub> by approximately 0.7% (not shown).
...
Table 12 compares test set F<sub>1</sub> scores of our ELMo enhanced biLSTM-CRF tagger with previous results. Overall, the 92.22% F<sub>1</sub> from our system establishes a new state-of-the-art. When compared to Peters et al. (2017), using representations
...
| Chiu and Nichols (2016) <sup>✦,◇</sup> | $91.62 \pm 0.33$                   |
| Peters et al. (2017) <sup>◇</sup>      | $91.93 \pm 0.19$                   |
| biLSTM-CRF + ELMo                      | <b><math>92.22 \pm 0.10</math></b> |

Table 12: Test set  $F_1$  for CoNLL 2003 NER task. Models with <sup>✦</sup> included gazetteers and those with <sup>◇</sup> used both the train and development splits for training.
