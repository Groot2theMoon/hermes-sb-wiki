---
ingested: 2026-04-30
sha256: 68297998cbe12aa231140d1752c8b8adf2b5b8b59854cf6b761750ddf67e3c41
---
...
We introduce a new language representation model called **BERT**, which stands for **Bi**directional **E**ncoder **R**epresentations from **T**ransformers. Unlike recent language representation models (Peters et al., 2018a; Radford et al., 2018), BERT is designed to pre-train deep bidirectional representations from unlabeled text by jointly conditioning on both left and right context in all layers. As a result, the pre-trained BERT model can be fine-tuned with just one additional output layer to create state-of-the-art models for a wide range of tasks, such as question answering and language inference, without substantial task-specific architecture modifications.
...
In this paper, we improve the fine-tuning based approaches by proposing **BERT: Bi**directional **E**ncoder **R**epresentations from **T**ransformers. BERT alleviates the previously mentioned unidirectionality constraint by using a “masked language model” (MLM) pre-training objective, inspired by the Cloze task (Taylor, 1953). The masked language model randomly masks some of the tokens from the input, and the objective is to predict the original vocabulary id of the masked
...
![Figure 1: Overall pre-training and fine-tuning procedures for BERT. The diagram is split into two main sections: Pre-training and Fine-Tuning. In the Pre-training section, an 'Unlabeled Sentence A and B Pair' is shown. Sentence A is processed through tokens [CLS], Token 1, [SEP], Token N, and [MASK]. Sentence B is processed through tokens [CLS], Token 1, [SEP], Token N, and [MASK]. These tokens are converted into embeddings (E_1, E_2, E_3, E_4, E_5) and passed through a BERT encoder. The output for the [MASK] tokens is used for 'Mask LM' (Masked Language Model) prediction. The output for the [CLS] token is used for 'NSP' (Next Sentence Prediction) prediction. In the Fine-Tuning section, three tasks are shown: MNLI, NER, and SQuAD. For MNLI, a 'Question Answer Pair' is shown. The question is processed through tokens [CLS], Token 1, [SEP], Token N, and the paragraph is processed through tokens [CLS], Token 1, [SEP], Token N, [SEP], Token 1, [SEP], Token N. These tokens are converted into embeddings (E_1, E_2, E_3, E_4, E_5) and passed through a BERT encoder. The output for the start and end tokens is used for 'Start/End Span' prediction. The same BERT architecture is used for all tasks, with different output layers added on top.](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)

Figure 1: Overall pre-training and fine-tuning procedures for BERT. The diagram is split into two main sections: Pre-training and Fine-Tuning. In the Pre-training section, an 'Unlabeled Sentence A and B Pair' is shown. Sentence A is processed through tokens [CLS], Token 1, [SEP], Token N, and [MASK]. Sentence B is processed through tokens [CLS], Token 1, [SEP], Token N, and [MASK]. These tokens are converted into embeddings (E\_1, E\_2, E\_3, E\_4, E\_5) and passed through a BERT encoder. The output for the [MASK] tokens is used for 'Mask LM' (Masked Language Model) prediction. The output for the [CLS] token is used for 'NSP' (Next Sentence Prediction) prediction. In the Fine-Tuning section, three tasks are shown: MNLI, NER, and SQuAD. For MNLI, a 'Question Answer Pair' is shown. The question is processed through tokens [CLS], Token 1, [SEP], Token N, and the paragraph is processed through tokens [CLS], Token 1, [SEP], Token N, [SEP], Token 1, [SEP], Token N. These tokens are converted into embeddings (E\_1, E\_2, E\_3, E\_4, E\_5) and passed through a BERT encoder. The output for the start and end tokens is used for 'Start/End Span' prediction. The same BERT architecture is used for all tasks, with different output layers added on top.
...
![Figure 2: BERT input representation diagram. The diagram shows the construction of the input representation for the sentence 'my dog is cute he likes play #ing'. The input tokens are: [CLS], my, dog, is, cute, [SEP], he, likes, play, #ing, [SEP]. The Token Embeddings are: E_{[CLS]}, E_{my}, E_{dog}, E_{is}, E_{cute}, E_{[SEP]}, E_{he}, E_{likes}, E_{play}, E_{#ing}, E_{[SEP]}. The Segment Embeddings are: E_A, E_A, E_A, E_A, E_A, E_A, E_B, E_B, E_B, E_B, E_B. The Position Embeddings are: E_0, E_1, E_2, E_3, E_4, E_5, E_6, E_7, E_8, E_9, E_{10}. The final input representation is the sum of the token embeddings, segment embeddings, and position embeddings for each token.](f9a14fbfecbd7d059226cc93677d721b_img.jpg)
...
Figure 2: BERT input representation diagram. The diagram shows the construction of the input representation for the sentence 'my dog is cute he likes play #ing'. The input tokens are: [CLS], my, dog, is, cute, [SEP], he, likes, play, #ing, [SEP]. The Token Embeddings are: E\_{[CLS]}, E\_{my}, E\_{dog}, E\_{is}, E\_{cute}, E\_{[SEP]}, E\_{he}, E\_{likes}, E\_{play}, E\_{#ing}, E\_{[SEP]}. The Segment Embeddings are: E\_A, E\_A, E\_A, E\_A, E\_A, E\_A, E\_B, E\_B, E\_B, E\_B, E\_B. The Position Embeddings are: E\_0, E\_1, E\_2, E\_3, E\_4, E\_5, E\_6, E\_7, E\_8, E\_9, E\_{10}. The final input representation is the sum of the token embeddings, segment embeddings, and position embeddings for each token.
...
$s_{i,j}^* = \max_{j \geq i} S \cdot T_i + E \cdot T_j$ . We predict a non-null answer when  $s_{i,j}^* > s_{null} + \tau$ , where the threshold  $\tau$  is selected on the dev set to maximize F1. We did not use TriviaQA data for this model. We fine-tuned for 2 epochs with a learning rate of 5e-5 and a batch size of 48.
...
- Daniel Cer, Mona Diab, Eneko Agirre, Inigo Lopez-Gazpizio, and Lucia Specia. 2017. Semeval-2017 task 1: Semantic textual similarity multilingual and crosslingual focused evaluation. In *Proceedings of the 11th International Workshop on Semantic Evaluation (SemEval-2017)*, pages 1–14, Vancouver, Canada. Association for Computational Linguistics.
...
- Felix Hill, Kyunghyun Cho, and Anna Korhonen. 2016. Learning distributed representations of sentences from unlabeled data. In *Proceedings of the 2016 Conference of the North American Chapter of the Association for Computational Linguistics: Human Language Technologies*. Association for Computational Linguistics.
...
![Figure 3: Differences in pre-training model architectures. The diagram shows three models: BERT (Ours), OpenAI GPT, and ELMo. BERT uses a bidirectional Transformer architecture where each token's representation is influenced by both its left and right context. OpenAI GPT uses a left-to-right Transformer architecture. ELMo uses a concatenation of independently trained left-to-right and right-to-left LSTMs to generate features for downstream tasks.](af7916c89a458fdab6c3f443217388ae_img.jpg)

Figure 3: Differences in pre-training model architectures. The diagram shows three models: BERT (Ours), OpenAI GPT, and ELMo. BERT uses a bidirectional Transformer architecture where each token's representation is influenced by both its left and right context. OpenAI GPT uses a left-to-right Transformer architecture. ELMo uses a concatenation of independently trained left-to-right and right-to-left LSTMs to generate features for downstream tasks.
...
We train with batch size of 256 sequences (256 sequences  $\times$  512 tokens = 128,000 tokens/batch) for 1,000,000 steps, which is approximately 40
...
Here we studied the differences in recent popular representation learning models including ELMo, OpenAI GPT and BERT. The comparisons between the model architectures are shown visually in Figure 3. Note that in addition to the architecture differences, BERT and OpenAI GPT are fine-tuning approaches, while ELMo is a feature-based approach.
...
![Figure 4: Illustrations of Fine-tuning BERT on Different Tasks. The figure contains four sub-diagrams: (a) Sentence Pair Classification Tasks, (b) Single Sentence Classification Tasks, (c) Question Answering Tasks, and (d) Single Sentence Tagging Tasks. Each diagram shows the input tokenization, the BERT model, and the output head for a specific task.](33ed1f9b27c7c21c797aa928b0f06851_img.jpg)
...
Figure 4: Illustrations of Fine-tuning BERT on Different Tasks. The figure contains four sub-diagrams: (a) Sentence Pair Classification Tasks, (b) Single Sentence Classification Tasks, (c) Question Answering Tasks, and (d) Single Sentence Tagging Tasks. Each diagram shows the input tokenization, the BERT model, and the output head for a specific task.
...
![Figure 5: A line graph showing MNLI Dev Accuracy (Y-axis, 76 to 84) versus Pre-training Steps (Thousands) (X-axis, 0 to 1,000). Two lines are plotted: BERTBASE (Masked LM) in blue with triangle markers and BERTBASE (Left-to-Right) in red with cross markers. Both lines start at approximately 79.0 accuracy at 100k steps. The blue line rises more steeply, reaching about 83.5 at 600k steps and 84.0 at 1,000k steps. The red line rises more gradually, reaching about 81.8 at 600k steps and 82.0 at 1,000k steps.](02ab1c6abcb5a2d487a30d019a5454b6_img.jpg)

| Pre-training Steps (Thousands) | BERT <sub>BASE</sub> (Masked LM) Accuracy | BERT <sub>BASE</sub> (Left-to-Right) Accuracy |
|--------------------------------|-------------------------------------------|-----------------------------------------------|
| 100                            | 79.0                                      | 79.0                                          |
| 200                            | 81.5                                      | 80.5                                          |
| 400                            | 82.5                                      | 81.5                                          |
...
Figure 5: A line graph showing MNLI Dev Accuracy (Y-axis, 76 to 84) versus Pre-training Steps (Thousands) (X-axis, 0 to 1,000). Two lines are plotted: BERTBASE (Masked LM) in blue with triangle markers and BERTBASE (Left-to-Right) in red with cross markers. Both lines start at approximately 79.0 accuracy at 100k steps. The blue line rises more steeply, reaching about 83.5 at 600k steps and 84.0 at 1,000k steps. The red line rises more gradually, reaching about 81.8 at 600k steps and 82.0 at 1,000k steps.
