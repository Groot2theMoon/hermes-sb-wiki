---
ingested: 2026-04-30
sha256: 5ffed63d2d02e8d24c769e76d028d3790877211e5e4ba88f4215c5406476ee26
---
...
**Portfolio Construction:** To eliminate market-wide volatility risk and isolate stock selection capability, we adopt a market-neutral strategy. Specifically, the portfolio holds an equal number of stocks for both long (buy) and short (sell) positions with equal weighting. **Rebalancing Frequency:** We conduct portfolio rebalancing at the opening of the first business day of each month.
...
![Figure 1: Overview of our multi-agent LLM trading system. The diagram illustrates a three-level process. Level 1, 'Information Aggregation and Scoring', takes input data (Stock prices, Financial statements, News texts, Macroeconomics data) and processes them through four agents: Technical Agent, Fundamental Agent (Quantitative), Fundamental Agent (Qualitative), and News Agent. Each agent produces a 'Score & Report'. Level 2, 'Sector- and Macro-Level Adjustment', takes these scores and reports and processes them through a Sector Agent (which compares stocks with sector average) and a Macro Agent. Both produce 'Adjusted Score & Report'. Level 3, 'Portfolio Construction', takes these adjusted scores and reports and processes them through a PM Agent to produce the 'Final score for each stock'.](2fa4a1bf91d0f34e87c689fbc1211fe3_img.jpg)
...
**Oscillator: Moving Average Convergence Divergence (MACD):** This metric measures trend momentum as the difference between short-term (12-day) and long-term (26-day) exponential moving averages (EMA) of price, with a 9-day EMA of the MACD line as the "signal line," and their difference as the "histogram." We show the detailed formulations in Appendix A.
...
![Figure 2: Box plot showing Sharpe ratios for fine-grained (pink) and coarse-grained (blue) settings across portfolio sizes (10, 20, 30, 40, 50). The y-axis represents the Sharpe ratio, ranging from 0.0 to 2.0. The x-axis represents the Portfolio size. For each portfolio size, the fine-grained setting (pink) generally shows a higher Sharpe ratio than the coarse-grained setting (blue). Significance levels are indicated by stars: ns (not significant) for 10 stocks, *** for 20 stocks, * for 30 stocks, **** for 40 stocks, and **** for 50 stocks.](d6226b6d22b1babc1eeeb93e07cc9cc4_img.jpg)

| Portfolio Size | Fine-grained (Pink) Median | Coarse-grained (Blue) Median | Significance |
|----------------|----------------------------|------------------------------|--------------|
| 10             | ~0.5                       | ~0.6                         | ns           |
| 20             | ~0.8                       | ~0.6                         | ***          |
| 30             | ~0.8                       | ~0.6                         | *            |
...
Figure 2: Box plot showing Sharpe ratios for fine-grained (pink) and coarse-grained (blue) settings across portfolio sizes (10, 20, 30, 40, 50). The y-axis represents the Sharpe ratio, ranging from 0.0 to 2.0. The x-axis represents the Portfolio size. For each portfolio size, the fine-grained setting (pink) generally shows a higher Sharpe ratio than the coarse-grained setting (blue). Significance levels are indicated by stars: ns (not significant) for 10 stocks, \*\*\* for 20 stocks, \* for 30 stocks, \*\*\*\* for 40 stocks, and \*\*\*\* for 50 stocks.
...
The covariance structure among the six agent strategies is derived from the stock-level covariance matrix of TOPIX 100 constituents. Letting  $V \in \mathbb{R}^{n \times n}$  denote the stock covariance matrix and  $P \in \mathbb{R}^{M \times n}$  the portfolio weight matrix whose rows correspond to each agent’s stock-level holdings, the agent-level covariance is computed as  $\Sigma = PVP^T$ . To combine the six strategies into a single composite, we solve for the weight vector  $w$  that equalizes each agent’s risk contribution to total portfolio variance: each asset’s contribution  $w_i(\Sigma w)_i / \sqrt{w^T \Sigma w}$  is driven toward  $1/N$  via constrained optimization, subject to  $\sum_i w_i = 1$ .

We then vary the allocation ratio between TOPIX 100 and the agent composite from 0% to 100% in 10% increments, evaluating out-of-sample performance including annualized return, volatility, and Sharpe ratio, net of realistic transaction costs (10bps one-way). A key empirical finding is that the correlation between the TOPIX 100 index returns and the agent composite returns is low ( $\approx 0.4$ ), creating substantial diversification benefits. As Figure 3 shows, blended portfolios consistently achieve higher Sharpe ratios than either the TOPIX 100 index or the agent composite alone (The return and volatility are shown in Table 4 in Appendix C). While the ex-ante optimal allocation ratio is unknown in practice, even a naive 50%/50% split between the index and the agent composite yields a Sharpe ratio superior to both standalone components—demonstrating that practitioners can capture meaningful risk-adjusted performance improvements without requiring precise allocation optimization.
...
![Figure 3: A line graph showing the Sharpe ratio as a function of agent strategy allocation (%) between TOPIX 100 and the aggregated agent strategy in the test period. The x-axis represents 'Agent strategy allocation (%)' from 0% to 100% in 20% increments. The y-axis represents 'Sharpe ratio' from 1.00 to 2.00 in 0.25 increments. Two lines are plotted: 'Gross' (orange line with circular markers) and 'Net (10bps)' (green line with circular markers). Both lines start at approximately 1.85 at 0% allocation, peak at around 2.05 at 40% allocation, and then decline to approximately 1.15 at 100% allocation. The 'Gross' line is consistently above the 'Net' line throughout the graph.](3121afa7ca030b22ee0345864ca6f38b_img.jpg)

| Agent strategy allocation (%) | Gross Sharpe ratio | Net (10bps) Sharpe ratio |
|-------------------------------|--------------------|--------------------------|
| 0%                            | ~1.85              | ~1.85                    |
| 20%                           | ~1.95              | ~1.90                    |
| 40%                           | ~2.05              | ~2.00                    |
...
Figure 3: A line graph showing the Sharpe ratio as a function of agent strategy allocation (%) between TOPIX 100 and the aggregated agent strategy in the test period. The x-axis represents 'Agent strategy allocation (%)' from 0% to 100% in 20% increments. The y-axis represents 'Sharpe ratio' from 1.00 to 2.00 in 0.25 increments. Two lines are plotted: 'Gross' (orange line with circular markers) and 'Net (10bps)' (green line with circular markers). Both lines start at approximately 1.85 at 0% allocation, peak at around 2.05 at 40% allocation, and then decline to approximately 1.15 at 100% allocation. The 'Gross' line is consistently above the 'Net' line throughout the graph.
...
- [2] Ran Aroesti. [n.d.]. ranaroussi/yfinance: Download market data from Yahoo! Finance’s API. <https://github.com/ranaroussi/yfinance?tab=readme-ov-file> [Online; accessed 2026-02-05].
- [3] Berk Atıl, Sarp Aykent, Alexa Chittams, Lisheng Fu, Rebecca J Passonneau, Evan Radcliffe, Guru Rajan Rajagopal, Adam Sloan, Tomasz Tudrej, Ferhan Ture, et al. 2024. Non-determinism of “deterministic” LLM settings. *arXiv preprint arXiv:2408.04667* (2024).
...
- [5] Tristan JB Canu, Ben Dennes, Travis Coan, Saffron O’Neill, and Hywel TP Williams. 2025. Using semantic similarity to measure the echo of strategic communications. *EPJ Data Science* 14, 1 (2025), 20.
...
- [9] Yifei Dong, Pengxi Wu, Kunlun Zhang, Yilong Dai, Sanjian Zhang, Wanghao Ye, Sihan Chen, and Zhi-Qi Cheng. 2025. Large Language Model Agents in Finance: A Survey Bridging Research, Practice, and Real-World Deployment. In *Findings of the Association for Computational Linguistics: EMNLP 2025*. 17889–17907.

- [10] Felix Drinckli, Egibal Rahimikia, Janet Pierrehumbert, and Stefan Zohren. 2024. Time machine GPT. In *Findings of the Association for Computational Linguistics: NAACL 2024*. 3383–3392.
- [11] EDINET. [n.d.]. EDINET. <https://disclosure.e1.edinet-fsa.go.jp/guide/static/disclosure/WEK0060.html> [Online; accessed 2026-02-05].
- [12] FRED. [n.d.]. US Fed. Line Web Services: FRED® API. <https://fred.stlouisfed.org/docs/api/fred/> [Online; accessed 2026-02-05].
...
- [14] Sirui Hong, Mingchen Zhuge, Jonathan Chen, Xiuwu Zheng, Yuheng Cheng, Jinlin Wang, Ceyao Zhang, Zili Wang, Steven Ka Shing Yau, Zijuan Lin, et al. 2023. MetaGPT: Meta programming for a multi-agent collaborative framework. In *The Twelfth International Conference on Learning Representations*.
...
- [16] Yoonjae Hwang, Yuxuan Kong, Stefan Zohren, and Yongjie Lee. 2025. Decision-informed neural networks with large language model integration for portfolio optimization. *arXiv preprint arXiv:2502.00828* (2025).
- [17] Yerin Hwang, Dongryeol Lee, Taegwan Kang, Minwoo Lee, and Kyomin Jung. 2026. When Wounding Steers the Evaluation: Framing Bias in LLM Judges. *arXiv preprint arXiv:2601.13537* (2026).
- [18] Aakansha Jadhav and Vishal Mirza. 2025. Large Language Models in Equity Markets: Applications, Techniques, and Insights. *Frontiers in Artificial Intelligence* Volume 8 - 2025 (2025).
- [19] Ranira Khojato, Mazen Mohamad, Linda Erlenhow, Francisco Gomes de Oliveira Neto, and Philipp Leitner. 2025. LLM Company Policies and Policy Implications in Software Organizations. *IEEE Software* (2025).
...
- [21] Hyungjoo Joon Kim, Yoonsa Kim, Cheonbok Park, Junyeob Kim, Choonghyun Park, Kang Min Yoo, Sang-goo Kim, and Taek Kim. 2024. Aligning language models to explicitly handle ambiguity. *arXiv preprint arXiv:2404.11972* (2024).
- [22] Harvey Bonmu Ku, Jeongyeol Shin, Hyoun Jun Lee, Seonok Na, and Insoo Jeon. 2025. Multi-agent LLM debate unveils the premise left unsaid. In *Proceedings of the 12th Argument mining Workshop*. 58–73.
...
- [26] Hoyoung Lee, Junhyuk Seo, Sulwan Park, Junhyeong Lee, Wonbin Ahn, Chanwook Choi, Alejandro Lopez-Lira, and Yongjae Lee. 2025. Your AI not your view: The bias of LLMs in investment analysis. In *Proceedings of the 6th ACM International Conference on AI in Finance*. 150–158.
- [27] Guohao Li, Hassan Hammoud, Hani Itani, Dimitri Khishullin, and Bernard Ghanem. 2023. Camel: Communicative agents for "mind" exploration of large language model society. *Advances in Neural Information Processing Systems* 36 (2023). 5199–5208.
...
- [29] Xiangyu Li, Yawen Zeng, Xiaofen Xing, Jin Xu, and Xiangmin Xu. 2025. Hedgegents: A balanced-aware multi-agent financial trading system. In *Companion Proceedings of the ACM on Web Conference 2025*. 296–305.
- [30] Yawei Li, Peipei Liu, and Ze Wang. 2022. Stock trading strategies based on deep reinforcement learning. *Scientific Programming* 2022, 1 (2022), 469856.
...
- [38] Sina Montazeri, Haseebullah Jumakhan, and Amir Mirzaeian. 2025. Finding Optimal Trading History in Reinforcement Learning for Stock Market Trading. *arXiv preprint arXiv:2502.12537* (2025).
- [39] Yuyi Nie, Yuxuan Kong, Xiaowen Dong, John M Mulvey, H Vincent Poor, Qingsong Wen, and Stefan Zohren. 2024. A survey of large language models for financial applications: Progress, prospects and challenges. *arXiv preprint arXiv:2406.11903* (2024).
- [40] Katsuhiko Ohkda, Moe Nakasui, Yasutomo Tsukioka, and Takahiro Yamasaki. 2025. From words to returns: sentiment analysis of Japanese 10-K reports using advanced large language models. *PeerJ Computer Science* 11 (2025), e3349.
...
- [44] George Papageorgiou, Dimitrios Gkaiminis, and Christos Tjortjis. 2024. Enhancing stock market forecasts with double deep q-network in volatile stock market environments. *Electronics* 13, 9 (2024), 1629.
...
- [46] Sida Peng, Eirini Kalliamvakou, Peter Gilon, and Mert Demirel. 2023. The impact of AI on developer productivity: Evidence from github copilot. *arXiv preprint arXiv:2302.06590* (2023).
...
- [49] Linbin Qiu, Yuhang Ye, Zhirong Gao, Xide Zou, Junfu Chen, Ziming Gui, Weizhi Han, Xiangyang Yue, Wenqi Xu, and Kun Zhao. 2025. Blueprint First. Model Second: A Framework for Deterministic LLM Workflow. *arXiv preprint arXiv:2508.02721* (2025).
...
- [52] Parshin Shojaee, Iman Mirzadeh, Keivan Alizadeh, Maxwell Horton, Samy Bengio, and Mehrdad Farajtabar. 2025. The illusion of thinking: Understanding the strengths and limitations of reasoning models via the lens of problem complexity. *arXiv preprint arXiv:2506.06941* (2025).
- [53] Alex Singla, Alexander Sukharevsky, Lareina A. Yee, and Michael Chui. 2025. The State of AI: Global Survey 2025 | McKinsey. <https://www.mckinsey.com/capabilities/quanta/black/out-insights/the-state-of-ai/> [Online; accessed 2025-11-28].
...
- [56] Taichi Hashimoto Toshinori Sato and Manabu Okumura. 2017. Implementation of a word segmentation dictionary called mecab-ipadic-NEologd and study on how to use it effectively for information retrieval (in Japanese). In *Proceedings of the Twenty-Three Annual Meeting of the Association for Natural Language Processing*. The Association for Natural Language Processing, NLP2017–Be-1.
- [57] Lei Wang, Wanyu Xu, Yihuai Lan, Zhizhang Hu, Yunshi Lan, Roy Ka-Wei Lee, and Pei-Peng Lim. 2023. Plan-and-solve prompting: Improving zero-shot chain-of-thought reasoning by large language models. *arXiv preprint arXiv:2305.04091* (2023).
- [58] Saizhuo Wang, Hang Yuan, Leen Zhou, Lionel Ni, Heung Yeung Shum, and Jian Guo. 2025. Alpha-gpt: Human-ai interactive alpha mining for quantitative investment. In *Proceedings of the 2025 Conference on Empirical Methods in Natural Language Processing: System Demonstrations*. 196–206.
- [59] Xuezhi Wang, Jason Wei, Dale Schuurmans, Quoc Le, Ed Chi, Sharan Narang, Aakansha Chowdhery, and Denny Zhou. 2022. Self-consistency improves chain of thought reasoning in language models. *arXiv preprint arXiv:2203.11171* (2022).

- [60] Jiaxin Wen, Ruiqi Zhong, Akbir Khan, Ethan Perez, Jacob Steinhardt, Minlie Huang, Samuel R Bowman, He He, and Shi Feng. 2024. Language models learn to mislead humans via RLHF. *arXiv preprint arXiv:2409.12622* (2024).
- [61] Zeyu Xia, Mingde Shi, and Changle Liu. 2023. Stock trading strategy developing based on reinforcement learning. In *Proceedings of the 2nd International Academic Conference on Blockchain, Information Technology and Smart Finance (ICBIS 2023)*. Atlantis Press, 156–164.
...
- [66] Shunyu Yao, Jeffrey Zhao, Dian Yu, Nan Du, Ishak Shafraj, Karthik R Narasimhan, and Yuan Cao. 2022. React: Synergizing reasoning and acting in language models. In *The eleventh international conference on learning representations*.
- [67] Yangyang Yu, Haohang Li, Zhi Chen, Yuechen Jiang, Yang Li, Jordan W Suchow, Denghui Zhang, and Khaldoon Khashanah. 2025. Finnem: A performance-enhanced LLM trading agent with layered memory and character design. *IEEE Transactions on Big Data* (2025).
...
- [70] Wentao Zhang, Lingxuan Zhao, Haochong Xia, Shuo Sun, Jiaze Sun, Molei Qin, Xinyi Li, Yuqing Zhao, Yilei Zhao, Xinyu Cai, et al. 2024. A multimodal foundation agent for financial trading: To augmented, diversified, and generalist. In *Proceedings of the 30th acm sigkdd conference on knowledge discovery and data mining*. 4314–4325.
- [71] Haiyan Zhao, Hanjin Chen, Fan Yang, Ninghao Liu, Huiqi Deng, Hengyi Cai, Shuaiqiang Wang, Dawei Yin, and Mengnan Du. 2024. Explainability for large language models: A survey. *ACM Transactions on Intelligent Systems and Technology* 15, 2 (2024), 1–38.
- [72] Tianjiao Zhao, Jingrao Lyu, Stokes Jones, Harrison Garber, Stefano Pasquali, and Dheeraj Mehta. 2025. AlphaAgents: Large Language Model based Multi-Agents for Equity Portfolio Constructions. *arXiv preprint arXiv:2508.11152* (2025).
...
- [75] Jialong Zhao, Lichao Wang, and Xiao Yang. 2025. GUARDIAN: Safeguarding LLM Multi-Agent Collaborations with Temporal Graph Modeling. *arXiv preprint arXiv:2505.19234* (2025).
- [76] Mingchen Zhuge, Haozhe Liu, Francesco Faccio, Dylan R Ashley, Robert Csordás, Anand Gopalakrishnan, Abdullah Hamdi, Hasan Abdel Al Kader Hammoud, Vincent Herrmann, Kazuki Irie, et al. 2023. Mindstorms in natural language-based societies of mind. *arXiv preprint arXiv:2305.17066* (2023).
...
- State the reason in Japanese in one sentence ( $\approx 50$  chars).
...
RoC 20day: <value>% RoC Month: <value>%  
...
- State the reason in Japanese ( $\approx 50$  chars).
...
[<price\_t>, <price\_t-1>, ..., <price\_t-251>]  
...
- **Output:** Score (0-100) and reason (in Japanese,  $\approx 50$  chars).
...
- **Output:** JSON format with a score (0-100) and a Japanese reason ( $\approx 50$  chars).
...
##### **2. Sector & Comparative Context (Variable Inputs):** The inputs below switch between Coarse-grained and Fine-grained settings.
...
**Output Format:** JSON with "score" and "investment\_thesis".
...
```
{
  "metrics": { "market_trend": "...", "score": 0-100, ... },
  "summary": "<Implications for active management (approx 200 chars)>"
}
```

**Macro Indicators [Format: Value (RoC: ValueX)]:**
...
| 50-50 Combined   | 16.8% | 8.0%  | <b>2.11</b> | 15.2%       | 8.0%  | <b>1.91</b> |
...
|------------------------|-------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| Technical<br>(Level 1) | Momentum, Neutral, Short-term, Favorable, Implication, Suggestion, Long-term, Condition, Decline, Volatility            | Price, Trend, Rise, Upward-trend, Fall, Stock-price, Recent, Recommend, Continuation, Expectation        |
| Quant.<br>(Level 1)    | Soundness, Margins, Growth, Profitability, Growth-rate, Favorable, Deterioration, Financials, Issues, Unstable          | EPS, Stability, Improvement, Profit, Earnings, Trend, Stagnation, Increase, Attractive, Stability        |
| Sector<br>(Level 2)    | Momentum, Soundness, Financials, Margins, Profitability, Growth-rate, Indicators, ROE, Efficiency, Undervalued          | Overall, Trend, EPS, Improvement, Operating-profit, Stagnation, Net-income, Stock-price, Trend, Decrease |
| PM<br>(Level 3)        | Momentum, Soundness, Financials, Profitability, Undervalued, Issues, Undervaluation, Margins, Overvaluation, Attractive | Sector, Trend, Improvement, Stagnation, EPS, Average, Stability, Decrease, Target, Downward              |