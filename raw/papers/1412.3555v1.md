---
ingested: 2026-04-30
sha256: 800db33c6cf70ce250b9d76c132e144628c5c0d5196b22018050397c71c56eeb
---
...
More formally, given a sequence  $\mathbf{x} = (\mathbf{x}_1, \mathbf{x}_2, \dots, \mathbf{x}_\tau)$ , the RNN updates its recurrent hidden state  $h_t$  by

$$\mathbf{h}_t = \begin{cases} 0, & t = 0 \\ \phi(\mathbf{h}_{t-1}, \mathbf{x}_t), & \text{otherwise} \end{cases} \quad (1)$$

where  $\phi$  is a nonlinear function such as composition of a logistic sigmoid with an affine transformation. Optionally, the RNN may have an output  $\mathbf{y} = (y_1, y_2, \dots, y_\tau)$  which may again be of variable length.
...
$$p(x_1, \dots, x_\tau) = p(x_1)p(x_2 | x_1)p(x_3 | x_1, x_2) \cdots p(x_\tau | x_1, \dots, x_{\tau-1}), \quad (3)$$
...
![Figure 1: Illustration of (a) LSTM and (b) gated recurrent units. (a) Long Short-Term Memory (LSTM) unit diagram showing input (i), forget (f), and output (o) gates, memory cell (c), and new memory cell content (c-tilde). (b) Gated Recurrent Unit (GRU) diagram showing reset (r) and update (z) gates, activation (h), and candidate activation (h-tilde).](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)

(a) Long Short-Term Memory
(b) Gated Recurrent Unit

Figure 1: Illustration of (a) LSTM and (b) gated recurrent units. (a) Long Short-Term Memory (LSTM) unit diagram showing input (i), forget (f), and output (o) gates, memory cell (c), and new memory cell content (c-tilde). (b) Gated Recurrent Unit (GRU) diagram showing reset (r) and update (z) gates, activation (h), and candidate activation (h-tilde).
...
$$o_t^j = \sigma(W_o \mathbf{x}_t + U_o \mathbf{h}_{t-1} + V_o \mathbf{c}_{t-1})^j,$$
...
where  $r_t$  is a set of reset gates and  $\odot$  is an element-wise multiplication. <sup>1</sup> When off ( $r_t^j$  close to 0), the reset gate effectively makes the unit act as if it is reading the first symbol of an input sequence, allowing it to *forget* the previously computed state.
...
$$\tilde{h}_t^j = \tanh(W \mathbf{x}_t + r_t \odot (U \mathbf{h}_{t-1}))^j,$$
...
![Figure 2: Learning curves for training and validation sets of different types of units. The figure consists of four subplots arranged in a 2x2 grid. The top row shows negative-log likelihood vs. number of iterations (0 to 500). The bottom row shows negative-log likelihood vs. wall clock time (seconds). The left column is for the Nottingham Dataset, and the right column is for the MuseData Dataset. Each plot compares tanh, GRU, and LSTM units for training (solid lines) and validation (dashed lines). The y-axis is on a log scale. In all cases, GRU and LSTM units show faster convergence and lower final negative-log likelihood compared to the tanh unit. LSTM generally outperforms GRU, except in the MuseData dataset where GRU reaches a lower final loss than LSTM.](c0843c6d138705289960d9f53a6e72a1_img.jpg)
...
Figure 2: Learning curves for training and validation sets of different types of units. The figure consists of four subplots arranged in a 2x2 grid. The top row shows negative-log likelihood vs. number of iterations (0 to 500). The bottom row shows negative-log likelihood vs. wall clock time (seconds). The left column is for the Nottingham Dataset, and the right column is for the MuseData Dataset. Each plot compares tanh, GRU, and LSTM units for training (solid lines) and validation (dashed lines). The y-axis is on a log scale. In all cases, GRU and LSTM units show faster convergence and lower final negative-log likelihood compared to the tanh unit. LSTM generally outperforms GRU, except in the MuseData dataset where GRU reaches a lower final loss than LSTM.
...
![Figure 3: Learning curves for training and validation sets of different types of units. The figure consists of four subplots arranged in a 2x2 grid. The top row shows 'Per epoch' learning curves where the x-axis is the number of iterations (0 to 800) and the y-axis is the negative-log likelihood on a log-scale (10^0 to 10^2). The bottom row shows 'Wall Clock Time (seconds)' learning curves where the x-axis is wall clock time (0 to 15000 for Dataset A, 0 to 20000 for Dataset B) and the y-axis is the negative-log likelihood on a log-scale (10^0 to 10^2). Each subplot compares four models: tanh (blue), GRU (teal), and LSTM (purple), each with a 'train' (solid line with circles) and 'valid' (dashed line with diamonds) series. In all cases, the LSTM model shows the lowest negative-log likelihood, indicating the best performance, while the tanh model generally shows the highest negative-log likelihood, indicating the worst performance. The GRU model's performance is intermediate between tanh and LSTM.](c3c305cefbac2e7b13be34ab87054d1e_img.jpg)

Figure 3: Learning curves for training and validation sets of different types of units. The figure consists of four subplots arranged in a 2x2 grid. The top row shows 'Per epoch' learning curves where the x-axis is the number of iterations (0 to 800) and the y-axis is the negative-log likelihood on a log-scale (10^0 to 10^2). The bottom row shows 'Wall Clock Time (seconds)' learning curves where the x-axis is wall clock time (0 to 15000 for Dataset A, 0 to 20000 for Dataset B) and the y-axis is the negative-log likelihood on a log-scale (10^0 to 10^2). Each subplot compares four models: tanh (blue), GRU (teal), and LSTM (purple), each with a 'train' (solid line with circles) and 'valid' (dashed line with diamonds) series. In all cases, the LSTM model shows the lowest negative-log likelihood, indicating the best performance, while the tanh model generally shows the highest negative-log likelihood, indicating the worst performance. The GRU model's performance is intermediate between tanh and LSTM.
