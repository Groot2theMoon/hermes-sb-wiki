---
ingested: 2026-04-30
sha256: b74ae087b7cd02ebfaf73e76796481bb74915d4ad58d8131912efc9e4c2f3dc0
---
...
![Diagram of a 5-layer dense block with a growth rate of k = 4. The diagram shows an 'input' stack of feature maps entering a series of five layers labeled 'BN-Relu-Conv'. Each layer produces a set of feature maps (H1, H2, H3, H4, H5) which are then passed to a 'Transition Layer'. The diagram illustrates dense connectivity: each layer takes all preceding feature-maps as input, shown by curved arrows connecting the output of one layer to the input of the next. The final output is a stack of feature maps labeled x5.](17b9d89b91e10c3efc38193baedee268_img.jpg)

Diagram of a 5-layer dense block with a growth rate of k = 4. The diagram shows an 'input' stack of feature maps entering a series of five layers labeled 'BN-Relu-Conv'. Each layer produces a set of feature maps (H1, H2, H3, H4, H5) which are then passed to a 'Transition Layer'. The diagram illustrates dense connectivity: each layer takes all preceding feature-maps as input, shown by curved arrows connecting the output of one layer to the input of the next. The final output is a stack of feature maps labeled x5.
...
![Figure 2: A schematic diagram of a deep DenseNet architecture. The process starts with an 'Input' image of a horse. It is followed by a 'Convolution' layer, then 'Dense Block 1'. Between 'Dense Block 1' and 'Dense Block 2' are a 'Convolution' and a 'Pooling' layer. Between 'Dense Block 2' and 'Dense Block 3' are another 'Convolution' and 'Pooling' layer. After 'Dense Block 3', there is a 'Pooling' layer followed by a 'Linear' layer, which outputs the 'Prediction' 'horse'. Each dense block is shown as a box containing five nodes with directed connections between them.](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)
...
Consider a single image  $\mathbf{x}_0$  that is passed through a convolutional network. The network comprises  $L$  layers, each of which implements a non-linear transformation  $H_\ell(\cdot)$ , where  $\ell$  indexes the layer.  $H_\ell(\cdot)$  can be a composite function of operations such as Batch Normalization (BN) [14], rectified linear units (ReLU) [6], Pooling [19], or Convolution (Conv). We denote the output of the  $\ell^{\text{th}}$  layer as  $\mathbf{x}_\ell$ .

**ResNets.** Traditional convolutional feed-forward networks connect the output of the  $\ell^{\text{th}}$  layer as input to the  $(\ell + 1)^{\text{th}}$  layer [16], which gives rise to the following layer transition:  $\mathbf{x}_\ell = H_\ell(\mathbf{x}_{\ell-1})$ . ResNets [11] add a skip-connection that bypasses the non-linear transformations with an identity function:
...
**ImageNet.** The ILSVRC 2012 classification dataset [2] consists 1.2 million images for training, and 50,000 for validation, from 1,000 classes. We adopt the same data augmentation scheme for training images as in [8, 11, 12], and apply a single-crop or 10-crop with size  $224 \times 224$  at test time. Following [11, 12, 13], we report classification errors on the validation set.
...
![Figure 3: Comparison of the DenseNets and ResNets top-1 error rates (single-crop testing) on the ImageNet validation dataset as a function of learned parameters (left) and FLOPs during test-time (right).](ed2fa033a401564314cdc32fe9732935_img.jpg)

Figure 3 consists of two line plots. The left plot shows 'validation error (%)' on the y-axis (ranging from 21.5 to 27.5) against '#parameters' on the x-axis (ranging from 1 to 8, scaled by 10^7). The right plot shows 'validation error (%)' on the y-axis (ranging from 21.5 to 27.5) against '#flops' on the x-axis (ranging from 0 to 2.5, scaled by 10^10). Both plots compare ResNets (red line with triangle markers) and DenseNets-BC (blue line with triangle markers). In both plots, DenseNets-BC consistently achieve lower validation error rates than ResNets for the same number of parameters or FLOPs. The models shown include ResNet-34, ResNet-50, ResNet-101, ResNet-152, DenseNet-121, DenseNet-169, DenseNet-201, and DenseNet-264.
...
![Figure 4: Comparison of parameter efficiency and training curves. Left: Test error (%) vs #parameters (x10^6) for DenseNet variants (DenseNet, DenseNet-C, DenseNet-B, DenseNet-BC). Middle: Test error (%) vs #parameters (x10^6) for ResNet and DenseNet-BC, highlighting that DenseNet-BC requires 5x fewer parameters for comparable accuracy. Right: Training and testing error (%) vs epoch for a 1001-layer ResNet-1001 (10.2M parameters) and a 100-layer DenseNet-BC (0.8M parameters).](4086a572c080354982c11f1de4d6921d_img.jpg)

Figure 4: Comparison of parameter efficiency and training curves. Left: Test error (%) vs #parameters (x10^6) for DenseNet variants (DenseNet, DenseNet-C, DenseNet-B, DenseNet-BC). Middle: Test error (%) vs #parameters (x10^6) for ResNet and DenseNet-BC, highlighting that DenseNet-BC requires 5x fewer parameters for comparable accuracy. Right: Training and testing error (%) vs epoch for a 1001-layer ResNet-1001 (10.2M parameters) and a 100-layer DenseNet-BC (0.8M parameters).
...
between the surrounding layers. As the pooling layers are never dropped, the network results in a similar connectivity pattern as DenseNet: there is a small probability for any two layers, between the same pooling layers, to be directly connected—if all intermediate layers are randomly dropped. Although the methods are ultimately quite different, the DenseNet interpretation of stochastic depth may provide insights into the success of this regularizer.
...
![Figure 5: Three heatmaps showing average absolute filter weights for Dense Block 1, Dense Block 2, and Dense Block 3. Each plot has 'Source layer (s)' on the y-axis and 'Target layer (l)' on the x-axis. The color scale ranges from 0.0 (blue) to 0.8 (red). Black rectangles highlight specific regions: the first row (transition layers) and the last column (classification layer). A red dot is visible in each plot at position (l, s) indicating strong dependency.](7ff005f9556dc6518981bb92091d36ab_img.jpg)
...
- [1] C.ortes, X. Gonzalvo, V. Kuznetsov, M. Mohri, and S. Yang. Adanet: Adaptive structural learning of artificial neural networks. *arXiv preprint arXiv:1607.01097*, 2016. 2
...
- [41] S. Yang and D. Ramanan. Multi-scale recognition with dag-cnn. In *ICCV*, 2015. 2
