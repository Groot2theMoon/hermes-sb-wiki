---
ingested: 2026-04-30
sha256: 8c431916215a916ccda41716aded7d0f3f6a8c75195e33b4b0ea9f54b323c786
---
...
![Figure 1: A directed graphical model diagram. A latent variable node 'z' is at the top, and an observed variable node 'x' is below it. A solid line with an arrow points from 'z' to 'x', representing the generative model p_theta(x|z). A dashed line with an arrow points from 'x' back to 'z', representing the variational approximation q_phi(z|x). An external parameter 'phi' has a dashed arrow pointing to the dashed line. An external parameter 'theta' has a solid arrow pointing to the solid line. The node 'x' is inside a rectangular plate labeled 'N' at the bottom, indicating N independent observations.](68ac34ff111db52afaa786afcb8346c3_img.jpg)

Figure 1: A directed graphical model diagram. A latent variable node 'z' is at the top, and an observed variable node 'x' is below it. A solid line with an arrow points from 'z' to 'x', representing the generative model p\_theta(x|z). A dashed line with an arrow points from 'x' back to 'z', representing the variational approximation q\_phi(z|x). An external parameter 'phi' has a dashed arrow pointing to the dashed line. An external parameter 'theta' has a solid arrow pointing to the solid line. The node 'x' is inside a rectangular plate labeled 'N' at the bottom, indicating N independent observations.
...
$$\log p_\theta(\mathbf{x}^{(i)}) \geq \mathcal{L}(\theta, \phi; \mathbf{x}^{(i)}) = \mathbb{E}_{q_\phi(\mathbf{z}|\mathbf{x}^{(i)})} [-\log q_\phi(\mathbf{z}|\mathbf{x}^{(i)}) + \log p_\theta(\mathbf{x}^{(i)}, \mathbf{z})] \quad (2)$$
...
$$\tilde{\mathbf{z}} = g_\phi(\epsilon, \mathbf{x}) \quad \text{with } \epsilon \sim p(\epsilon) \quad (4)$$
...
$$\mathbb{E}_{q_\phi(\mathbf{z}|\mathbf{x}^{(i)})} [f(\mathbf{z})] = \mathbb{E}_{p(\epsilon)} [f(g_\phi(\epsilon, \mathbf{x}^{(i)}))] \approx \frac{1}{L} \sum_{l=1}^L f(g_\phi(\epsilon^{(l)}, \mathbf{x}^{(i)})) \quad \text{where } \epsilon^{(l)} \sim p(\epsilon) \quad (5)$$
...
$$\tilde{\mathcal{L}}^A(\theta, \phi; \mathbf{x}^{(i)}) = \frac{1}{L} \sum_{l=1}^L \log p_\theta(\mathbf{x}^{(i)}, \mathbf{z}^{(i,l)}) - \log q_\phi(\mathbf{z}^{(i,l)}|\mathbf{x}^{(i)}) \quad (6)$$
...
$$\text{where } \mathbf{z}^{(i,l)} = g_\phi(\epsilon^{(i,l)}, \mathbf{x}^{(i)}) \text{ and } \epsilon^{(l)} \sim p(\epsilon) \quad (7)$$
...
1. Tractable inverse CDF. In this case, let  $\epsilon \sim \mathcal{U}(0, \mathbf{I})$ , and let  $q_\phi(\epsilon, \mathbf{x})$  be the inverse CDF of  $q_\phi(\mathbf{z}|\mathbf{x})$ . Examples: Exponential, Cauchy, Logistic, Rayleigh, Pareto, Weibull, Reciprocal, Gompertz, Gumbel and Erlang distributions.
...
<sup>2</sup>Note that this is just a (simplifying) choice, and not a limitation of our method.
...
![Figure 2: Comparison of AEVB and Wake-Sleep methods. The figure consists of ten subplots arranged in a 2x5 grid. The top row shows results for the MNIST dataset with latent space dimensions N_z = 3, 5, 10, 20, and 200. The bottom row shows results for the Frey Face dataset with latent space dimensions N_z = 2, 5, 10, and 20. Each plot shows the estimated average variational lower bound per datapoint (y-axis) against the number of training samples evaluated (x-axis, logarithmic scale from 10^0 to 10^4). Four lines are plotted in each: Wake-Sleep (test) as a dashed black line, Wake-Sleep (train) as a solid green line, AEVB (test) as a dashed red line, and AEVB (train) as a solid red line. In all cases, AEVB converges faster and reaches a higher lower bound than Wake-Sleep. The AEVB test and train lines are very close to each other, indicating good generalization.](4086a572c080354982c11f1de4d6921d_img.jpg)

Figure 2: Comparison of AEVB and Wake-Sleep methods. The figure consists of ten subplots arranged in a 2x5 grid. The top row shows results for the MNIST dataset with latent space dimensions N\_z = 3, 5, 10, 20, and 200. The bottom row shows results for the Frey Face dataset with latent space dimensions N\_z = 2, 5, 10, and 20. Each plot shows the estimated average variational lower bound per datapoint (y-axis) against the number of training samples evaluated (x-axis, logarithmic scale from 10^0 to 10^4). Four lines are plotted in each: Wake-Sleep (test) as a dashed black line, Wake-Sleep (train) as a solid green line, AEVB (test) as a dashed red line, and AEVB (train) as a solid red line. In all cases, AEVB converges faster and reaches a higher lower bound than Wake-Sleep. The AEVB test and train lines are very close to each other, indicating good generalization.
...
![Figure 3: Comparison of AEVB to the wake-sleep algorithm and Monte Carlo EM, in terms of the estimated marginal likelihood, for a different number of training points. The figure consists of two side-by-side line plots. The left plot is for N_train = 1000 and the right plot is for N_train = 50000. Both plots show 'Marginal log-likelihood' on the y-axis (ranging from -160 to -100) against '# Training samples evaluated (millions)' on the x-axis (ranging from 0 to 60). The legend indicates: Wake-Sleep (train) - solid green line, Wake-Sleep (test) - dashed green line, MCEM (train) - solid blue line, MCEM (test) - dashed blue line, AEVB (train) - solid red line, and AEVB (test) - dashed red line. In both plots, AEVB (train) starts at a high log-likelihood and increases rapidly, reaching a plateau around -110. Wake-Sleep (train) increases more slowly, reaching a plateau around -130. MCEM (train) starts at a lower log-likelihood and increases more slowly than AEVB. The test curves generally follow the train curves, with AEVB (test) showing the highest log-likelihood.](c3c305cefbac2e7b13be34ab87054d1e_img.jpg)

Figure 3: Comparison of AEVB to the wake-sleep algorithm and Monte Carlo EM, in terms of the estimated marginal likelihood, for a different number of training points. The figure consists of two side-by-side line plots. The left plot is for N\_train = 1000 and the right plot is for N\_train = 50000. Both plots show 'Marginal log-likelihood' on the y-axis (ranging from -160 to -100) against '# Training samples evaluated (millions)' on the x-axis (ranging from 0 to 60). The legend indicates: Wake-Sleep (train) - solid green line, Wake-Sleep (test) - dashed green line, MCEM (train) - solid blue line, MCEM (test) - dashed blue line, AEVB (train) - solid red line, and AEVB (test) - dashed red line. In both plots, AEVB (train) starts at a high log-likelihood and increases rapidly, reaching a plateau around -110. Wake-Sleep (train) increases more slowly, reaching a plateau around -130. MCEM (train) starts at a lower log-likelihood and increases more slowly than AEVB. The test curves generally follow the train curves, with AEVB (test) showing the highest log-likelihood.
...
![Figure 4: Visualisations of learned data manifold for generative models with two-dimensional latent space. (a) Learned Frey Face manifold: A 10x10 grid of small grayscale face images showing a smooth transition between different facial features. (b) Learned MNIST manifold: A 10x10 grid of handwritten digits (0-9) showing a smooth transition between different digit shapes.](adb1f42239329fa8283d1a40005f989f_img.jpg)

Figure 4: Visualisations of learned data manifold for generative models with two-dimensional latent space. (a) Learned Frey Face manifold: A 10x10 grid of small grayscale face images showing a smooth transition between different facial features. (b) Learned MNIST manifold: A 10x10 grid of handwritten digits (0-9) showing a smooth transition between different digit shapes.
...
![Figure 5: Random samples from learned generative models of MNIST for different dimensionalities of latent space. (a) 2-D latent space: A 10x10 grid of blurry, low-resolution handwritten digits. (b) 5-D latent space: A 10x10 grid of slightly sharper handwritten digits. (c) 10-D latent space: A 10x10 grid of clear handwritten digits. (d) 20-D latent space: A 10x10 grid of very clear and high-resolution handwritten digits.](7801d00a216dc4dc8a7d210dcb5fe3c5_img.jpg)

Figure 5: Random samples from learned generative models of MNIST for different dimensionalities of latent space. (a) 2-D latent space: A 10x10 grid of blurry, low-resolution handwritten digits. (b) 5-D latent space: A 10x10 grid of slightly sharper handwritten digits. (c) 10-D latent space: A 10x10 grid of clear handwritten digits. (d) 20-D latent space: A 10x10 grid of very clear and high-resolution handwritten digits.
...
$$\begin{aligned} \int q_\phi(\mathbf{z}) \log p(\mathbf{z}) d\mathbf{z} &= \int \mathcal{N}(\mathbf{z}; \boldsymbol{\mu}, \boldsymbol{\sigma}^2) \log \mathcal{N}(\mathbf{z}; 0, \mathbf{I}) d\mathbf{z} \\ &= -\frac{J}{2} \log(2\pi) - \frac{1}{2} \sum_{j=1}^J (\mu_j^2 + \sigma_j^2) \end{aligned}$$
...
$$\begin{aligned} -D_{KL}((q_\phi(\mathbf{z})||p_\theta(\mathbf{z}))) &= \int q_\phi(\mathbf{z}) (\log p_\theta(\mathbf{z}) - \log q_\theta(\mathbf{z})) d\mathbf{z} \\ &= \frac{1}{2} \sum_{j=1}^J (1 + \log((\sigma_j)^2) - (\mu_j)^2 - (\sigma_j)^2) \end{aligned}$$
...
$$\begin{aligned} \log p(\mathbf{x}|\mathbf{z}) &= \sum_{i=1}^D x_i \log y_i + (1 - x_i) \cdot \log(1 - y_i) \\ \text{where } \mathbf{y} &= f_\sigma(\mathbf{W}_2 \tanh(\mathbf{W}_1 \mathbf{z} + \mathbf{b}_1) + \mathbf{b}_2) \end{aligned} \quad (11)$$
...
$$\begin{aligned} \log p(\mathbf{x}|\mathbf{z}) &= \log \mathcal{N}(\mathbf{x}; \boldsymbol{\mu}, \boldsymbol{\sigma}^2 \mathbf{I}) \\ \text{where } \boldsymbol{\mu} &= \mathbf{W}_4 \mathbf{h} + \mathbf{b}_4 \\ \log \boldsymbol{\sigma}^2 &= \mathbf{W}_5 \mathbf{h} + \mathbf{b}_5 \\ \mathbf{h} &= \tanh(\mathbf{W}_3 \mathbf{z} + \mathbf{b}_3) \end{aligned} \quad (12)$$
...
As written in the paper, it is possible to perform variational inference on both the parameters  $\theta$  and the latent variables  $\mathbf{z}$ , as opposed to just the latent variables as we did in the paper. Here, we'll derive our estimator for that case.
...
where again the first RHS term is the KL divergence of the approximate from the true posterior, and  $\mathcal{L}(\boldsymbol{\theta}, \phi; \mathbf{x})$  is the variational lower bound of the marginal likelihood of datapoint  $i$ :

$$\mathcal{L}(\boldsymbol{\theta}, \phi; \mathbf{x}^{(i)}) = \int q_\phi(\mathbf{z}|\mathbf{x}) \left( \log p_\theta(\mathbf{x}^{(i)}|\mathbf{z}) + \log p_\theta(\mathbf{z}) - \log q_\phi(\mathbf{z}|\mathbf{x}) \right) dz \quad (16)$$
...
Under certain mild conditions outlined in section (see paper) for chosen approximate posteriors  $q_\phi(\boldsymbol{\theta})$  and  $q_\phi(\mathbf{z}|\mathbf{x})$  we can reparameterize conditional samples  $\tilde{\mathbf{z}} \sim q_\phi(\mathbf{z}|\mathbf{x})$  as

$$\tilde{\mathbf{z}} = g_\phi(\boldsymbol{\epsilon}, \mathbf{x}) \quad \text{with} \quad \boldsymbol{\epsilon} \sim p(\boldsymbol{\epsilon}) \quad (17)$$

where we choose a prior  $p(\boldsymbol{\epsilon})$  and a function  $g_\phi(\boldsymbol{\epsilon}, \mathbf{x})$  such that the following holds:

$$\begin{aligned} \mathcal{L}(\boldsymbol{\theta}, \phi; \mathbf{x}^{(i)}) &= \int q_\phi(\mathbf{z}|\mathbf{x}) \left( \log p_\theta(\mathbf{x}^{(i)}|\mathbf{z}) + \log p_\theta(\mathbf{z}) - \log q_\phi(\mathbf{z}|\mathbf{x}) \right) dz \\ &= \int p(\boldsymbol{\epsilon}) \left( \log p_\theta(\mathbf{x}^{(i)}|\mathbf{z}) + \log p_\theta(\mathbf{z}) - \log q_\phi(\mathbf{z}|\mathbf{x}) \right) \bigg|_{\mathbf{z}=g_\phi(\boldsymbol{\epsilon}, \mathbf{x}^{(i)})} d\boldsymbol{\epsilon} \end{aligned} \quad (18)$$

The same can be done for the approximate posterior  $q_\phi(\boldsymbol{\theta})$ :

$$\tilde{\boldsymbol{\theta}} = h_\phi(\boldsymbol{\zeta}) \quad \text{with} \quad \boldsymbol{\zeta} \sim p(\boldsymbol{\zeta}) \quad (19)$$
...
$$\begin{aligned} \mathcal{L}(\phi; \mathbf{X}) &= \int q_\phi(\boldsymbol{\theta}) (\log p_\theta(\mathbf{X}) + \log p_\alpha(\boldsymbol{\theta}) - \log q_\phi(\boldsymbol{\theta})) d\boldsymbol{\theta} \\ &= \int p(\boldsymbol{\zeta}) (\log p_\theta(\mathbf{X}) + \log p_\alpha(\boldsymbol{\theta}) - \log q_\phi(\boldsymbol{\theta})) \bigg|_{\boldsymbol{\theta}=h_\phi(\boldsymbol{\zeta})} d\boldsymbol{\zeta} \end{aligned} \quad (20)$$

For notational conciseness we introduce a shorthand notation  $f_\phi(\mathbf{x}, \mathbf{z}, \boldsymbol{\theta})$ :

$$f_\phi(\mathbf{x}, \mathbf{z}, \boldsymbol{\theta}) = N \cdot (\log p_\theta(\mathbf{x}|\mathbf{z}) + \log p_\theta(\mathbf{z}) - \log q_\phi(\mathbf{z}|\mathbf{x})) + \log p_\alpha(\boldsymbol{\theta}) - \log q_\phi(\boldsymbol{\theta}) \quad (21)$$

Using equations (20) and (18), the Monte Carlo estimate of the variational lower bound, given datapoint  $\mathbf{x}^{(i)}$ , is:

$$\mathcal{L}(\phi; \mathbf{X}) \simeq \frac{1}{L} \sum_{l=1}^L f_\phi(\mathbf{x}^{(l)}, g_\phi(\boldsymbol{\epsilon}^{(l)}, \mathbf{x}^{(l)}), h_\phi(\boldsymbol{\zeta}^{(l)})) \quad (22)$$

where  $\boldsymbol{\epsilon}^{(l)} \sim p(\boldsymbol{\epsilon})$  and  $\boldsymbol{\zeta}^{(l)} \sim p(\boldsymbol{\zeta})$ . The estimator only depends on samples from  $p(\boldsymbol{\epsilon})$  and  $p(\boldsymbol{\zeta})$  which are obviously not influenced by  $\phi$ , therefore the estimator can be differentiated w.r.t.  $\phi$ . The resulting stochastic gradients can be used in conjunction with stochastic optimization methods such as SGD or Adagrad [DHS10]. See algorithm 1 for a basic approach to computing stochastic gradients.

### F.1 Example

Let the prior over the parameters and latent variables be the centered isotropic Gaussian  $p_\alpha(\boldsymbol{\theta}) = \mathcal{N}(\mathbf{z}; \mathbf{0}, \mathbf{I})$  and  $p_\theta(\mathbf{z}) = \mathcal{N}(\mathbf{z}; \mathbf{0}, \mathbf{I})$ . Note that in this case, the prior lacks parameters. Let's also assume that the true posteriors are approximately Gaussian with an approximately diagonal covariance. In this case, we can let the variational approximate posteriors be multivariate Gaussians with a diagonal covariance structure:

$$\begin{aligned} \log q_\phi(\boldsymbol{\theta}) &= \log \mathcal{N}(\boldsymbol{\theta}; \boldsymbol{\mu}_\theta, \boldsymbol{\sigma}_\theta^2 \mathbf{I}) \\ \log q_\phi(\mathbf{z}|\mathbf{x}) &= \log \mathcal{N}(\mathbf{z}; \boldsymbol{\mu}_z, \boldsymbol{\sigma}_z^2 \mathbf{I}) \end{aligned} \quad (23)$$
...
for l is 1 to L do
  ...
  g ← g + 1/L ∇_ϕ f_ϕ(x, g_ϕ(ϵ, x), h_ϕ(ζ))
end for
return g
...
$$\begin{aligned}
q_\phi(\theta) & \quad \text{as} \quad \tilde{\theta} = \mu_\theta + \sigma_\theta \odot \zeta & \quad \text{where} \quad \zeta \sim \mathcal{N}(\mathbf{0}, \mathbf{I}) \\
q_\phi(\mathbf{z}|\mathbf{x}) & \quad \text{as} \quad \tilde{\mathbf{z}} = \mu_z + \sigma_z \odot \epsilon & \quad \text{where} \quad \epsilon \sim \mathcal{N}(\mathbf{0}, \mathbf{I})
\end{aligned}$$

With  $\odot$  we signify an element-wise product. These can be plugged into the lower bound defined above (eqs (21) and (22)).

In this case it is possible to construct an alternative estimator with a lower variance, since in this model  $p_\phi(\theta)$ ,  $p_\theta(\mathbf{z})$ ,  $q_\phi(\theta)$  and  $q_\phi(\mathbf{z}|\mathbf{x})$  are Gaussian, and therefore four terms of  $f_\phi$  can be solved analytically. The resulting estimator is:
...
& \quad + \frac{1}{2} \sum_{j=1}^J \left( 1 + \log((\sigma_{\theta,j}^{(l)})^2) - (\mu_{\theta,j}^{(l)})^2 - (\sigma_{\theta,j}^{(l)})^2 \right)
\end{aligned} \tag{24}$$
