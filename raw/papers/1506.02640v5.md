---
ingested: 2026-04-30
sha256: 8cd2d190f222b1cbe42ef06acce1b30abd88f198964ee684b7adb3efed1a6fc9
---
...
![Figure 1: The YOLO Detection System. The diagram shows a three-step process: (1) Resizing the input image to 448x448, (2) Running a single convolutional network, and (3) Non-max suppression. The input image shows a person and two dogs. The output image shows the same scene with bounding boxes around the person and dogs, labeled with their respective class probabilities.](7cea8cfa9ce0cdc9fe5f3f27384ed943_img.jpg)

Figure 1: The YOLO Detection System. The diagram shows a three-step process: (1) Resizing the input image to 448x448, (2) Running a single convolutional network, and (3) Non-max suppression. The input image shows a person and two dogs. The output image shows the same scene with bounding boxes around the person and dogs, labeled with their respective class probabilities.
...
![Figure 2: The Model. A diagram illustrating the YOLO detection process. On the left, an input image of a person on a bicycle is overlaid with a 5x5 grid. Arrows point from this grid to a 'Class probability map' in the center, which is a 5x5 grid of colored squares. From the probability map, arrows point to 'Bounding boxes + confidence' on the right, showing the original image with a yellow bounding box around the person. Finally, an arrow points to 'Final detections' on the far right, where the person is highlighted with a pink bounding box.](6d6fd934f0988fdb2e2dd22865e8d8ed_img.jpg)

Figure 2: The Model. A diagram illustrating the YOLO detection process. On the left, an input image of a person on a bicycle is overlaid with a 5x5 grid. Arrows point from this grid to a 'Class probability map' in the center, which is a 5x5 grid of colored squares. From the probability map, arrows point to 'Bounding boxes + confidence' on the right, showing the original image with a yellow bounding box around the person. Finally, an arrow points to 'Final detections' on the far right, where the person is highlighted with a pink bounding box.
...
![Figure 3: The Architecture. A diagram showing the flow of data through a neural network. It starts with an input of size 448x448x3. The first layer is a Conv. Layer (7x7x64, stride 2) followed by a Maxpool Layer (2x2, stride 2), resulting in a 112x112x3 feature map. This is followed by a series of layers: Conv. Layer (3x3x192), Maxpool Layer (2x2, stride 2) to 56x56x192; Conv. Layers (1x1x128, 3x3x256, 1x1x256), Maxpool Layer (2x2, stride 2) to 28x28x192; Conv. Layers (1x1x256, 3x3x512) repeated 4 times, followed by a Maxpool Layer (2x2, stride 2) to 14x14x1024; Conv. Layers (1x1x512, 3x3x1024) repeated 2 times, resulting in 14x14x1024; Conv. Layers (1x1x1024, 3x3x1024, 1x1x1024) repeated 2 times, resulting in 14x14x1024; a Conv. Layer (1x1x1024, stride 2) to 7x7x1024; a fully connected layer (Conn. Layer) to 4096 units; another fully connected layer (Conn. Layer) to 4096 units; and finally a Conv. Layer (1x1x30) resulting in a 7x7x30 output tensor.](b230b8f21d8e82d55c0d311c8c32ef73_img.jpg)
...
$$\begin{aligned}
& \lambda_{\text{coord}} \sum_{i=0}^{\mathcal{S}^2} \sum_{j=0}^B \mathbb{1}_{ij}^{\text{obj}} \left[ (x_i - \hat{x}_i)^2 + (y_i - \hat{y}_i)^2 \right] \\
& + \lambda_{\text{coord}} \sum_{i=0}^{\mathcal{S}^2} \sum_{j=0}^B \mathbb{1}_{ij}^{\text{obj}} \left[ (\sqrt{w_i} - \sqrt{\hat{w}_i})^2 + (\sqrt{h_i} - \sqrt{\hat{h}_i})^2 \right] \\
& + \sum_{i=0}^{\mathcal{S}^2} \sum_{j=0}^B \mathbb{1}_{ij}^{\text{obj}} (C_i - \hat{C}_i)^2 \\
& + \lambda_{\text{obj}} \sum_{i=0}^{\mathcal{S}^2} \sum_{j=0}^B \mathbb{1}_{ij}^{\text{noobj}} (C_i - \hat{C}_i)^2 \\
& + \sum_{i=0}^{\mathcal{S}^2} \mathbb{1}_i^{\text{obj}} \sum_{c \in \text{classes}} (p_i(c) - \hat{p}_i(c))^2 \quad (3)
\end{aligned}$$
...
![Figure 4: Error Analysis: Fast R-CNN vs. YOLO. Two pie charts comparing error distributions. Fast R-CNN: Correct: 71.6%, Background: 13.6%, Sim: 4.3%, Loc: 8.6%, Other: 1.9%. YOLO: Correct: 65.5%, Background: 4.75%, Sim: 6.75%, Loc: 19.0%, Other: 4.0%.](92c6488dfdf125bc1b7bb6a235394652_img.jpg)
...
Figure 4: Error Analysis: Fast R-CNN vs. YOLO. Two pie charts comparing error distributions. Fast R-CNN: Correct: 71.6%, Background: 13.6%, Sim: 4.3%, Loc: 8.6%, Other: 1.9%. YOLO: Correct: 65.5%, Background: 4.75%, Sim: 6.75%, Loc: 19.0%, Other: 4.0%.
...
| <b>Fast R-CNN + YOLO</b> | <b>70.7</b> | <b>83.4</b> | <b>78.5</b> | <b>73.5</b> | <b>55.8</b> | <b>43.4</b> | <b>78.1</b> | <b>73.1</b> | <b>89.4</b> | <b>49.4</b> | <b>75.5</b> | <b>57.0</b> | <b>87.5</b> | <b>80.9</b> | <b>81.0</b> | <b>74.7</b> | <b>41.8</b> | <b>71.5</b> | <b>68.5</b> | <b>82.1</b> | <b>67.2</b> |
| MR.CNN,SNN [11]          | 70.7        | 85.0        | 79.6        | 71.5        | 55.3        | 57.7        | 76.0        | 73.9        | 84.6        | 50.5        | 74.3        | 61.7        | 85.5        | 79.9        | 81.7        | 76.4        | 41.0        | 69.0        | 61.2        | 77.7        | 72.1        |
...
| UMICH,FGS,STRUCT         | 66.4        | 82.9        | 76.1        | 64.1        | 44.6        | 49.4        | 70.3        | 71.2        | 84.6        | 42.7        | 68.6        | 55.8        | <b>82.7</b> | 77.1        | 79.9        | 68.7        | 41.4        | 69.0        | 60.0        | 72.0        | 66.2        |
| NUS,NIN,C2000 [7]        | 63.8        | 80.2        | 73.8        | 61.9        | 43.7        | 43.0        | 70.3        | 67.6        | 80.7        | 41.9        | 69.7        | 51.7        | 78.2        | 75.2        | 76.9        | 65.1        | 38.6        | 68.3        | 58.0        | 68.7        | 63.3        |
| BabyLearning [7]         | 63.2        | 78.0        | 74.2        | 61.3        | 45.7        | 42.7        | 68.2        | 66.5        | 80.2        | 40.6        | 70.0        | 49.8        | 79.0        | 74.3        | 77.9        | 64.0        | 35.3        | 67.9        | 55.7        | 68.7        | 62.6        |
| NUS,NIN                  | 62.4        | 77.9        | 73.1        | 62.6        | 39.5        | 43.3        | 68.1        | 66.4        | 78.9        | 39.1        | 68.1        | 50.0        | 77.2        | 71.3        | 76.1        | 64.7        | 38.4        | 66.9        | 56.2        | 66.9        | 62.7        |
...
![Figure 5(a): Picasso Dataset precision-recall curves. The graph plots Precision (y-axis, 0.0 to 1.0) against Recall (x-axis, 0.0 to 1.0). Curves are shown for YOLO (magenta), DPM (orange), Poselets (green), R-CNN (blue), and D&T (red). YOLO shows the highest performance, reaching a precision of approximately 0.85 at a recall of 0.8. Other models show lower performance, with D&T being the lowest.](c3c305cefbac2e7b13be34ab87054d1e_img.jpg)

Figure 5(a): Picasso Dataset precision-recall curves. The graph plots Precision (y-axis, 0.0 to 1.0) against Recall (x-axis, 0.0 to 1.0). Curves are shown for YOLO (magenta), DPM (orange), Poselets (green), R-CNN (blue), and D&T (red). YOLO shows the highest performance, reaching a precision of approximately 0.85 at a recall of 0.8. Other models show lower performance, with D&T being the lowest.
...
![Figure 6: Qualitative Results. A 2x4 grid of images showing YOLO object detection results. The top row contains five artworks: 'The Scream' (detecting 'person'), 'The Hay Wainford' (detecting 'person', 'horse', 'person'), 'The Great Gatsby' (detecting 'person', 'person', 'person', 'person'), 'The Mona Lisa' (detecting 'person', 'cat'), and 'The Starry Night' (detecting 'person'). The bottom row contains four natural images: a bird (detecting 'bird'), a car (detecting 'car', 'bird'), a person on a boat (detecting 'person', 'boat'), and a group of people (detecting 'person', 'person', 'person', 'person').](af54a0b30d50f3f8620b331cdc59ae3a_img.jpg)

Figure 6: Qualitative Results. A 2x4 grid of images showing YOLO object detection results. The top row contains five artworks: 'The Scream' (detecting 'person'), 'The Hay Wainford' (detecting 'person', 'horse', 'person'), 'The Great Gatsby' (detecting 'person', 'person', 'person', 'person'), 'The Mona Lisa' (detecting 'person', 'cat'), and 'The Starry Night' (detecting 'person'). The bottom row contains four natural images: a bird (detecting 'bird'), a car (detecting 'car', 'bird'), a person on a boat (detecting 'person', 'boat'), and a group of people (detecting 'person', 'person', 'person', 'person').
...
- [12] S. Ginosa, D. Haas, T. Brown, and J. Malik. Detecting people in cubist art. In *Computer Vision–ECCV 2014 Workshops*, pages 101–116. Springer, 2014. 7
